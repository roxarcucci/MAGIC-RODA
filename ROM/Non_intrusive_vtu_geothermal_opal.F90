...
program non_intrusive_vtu_geothermal_opal
  use spud
  use fields
  use state_module
...
!#endif
  ...
  no_smolyak_nodes=60 !no use in this program, but keep it for consitency with interpolate_snapshots_nonintruve_material_poeperty.
  numphase=1
    total_dumps=15!count_dumps(dump_sampling_period)
    windows = 14
  ! m = 4
  ! sparse_max =1
  ! call get_option('/reduced_model/smolyak_order', sparse_max) 
     call nonintrusive_snapshots_main_geothermal_opal() 
 !  call nonintrusive_snapshots_main_geothermal_opal_time_windows()
...
contains 
  subroutine nonintrusive_snapshots_main_geothermal_opal()
    ...
    call put_vtu_to_state(state)
    call retrieve_nodesvalue_from_states_nowindows(state,  snapmatrix_pressure ) 
   ...
   ! allocate(snapmatrix_pressure(no_smolyak_nodes,p_nodes,snapshots,numphase)) 
    ...
    !allocate(interpolate_velocity(u_nodes,snapshots,dimen,numphase)) ! need more
     ...
     allocate(interpolate_pressure(p_nodes,snapshots,numphase))  !change on Saturday
   ...
   !  do k=1, no_smolyak_nodes
       do i=1, snapshots
      !  interpolate_pressure(:, snapshots*(k-1)+i,1)=snapmatrix_pressure(k,:,i,1) !-1000000
       interpolate_pressure(:, i,1)=snapmatrix_pressure(1,:,i,1)
     enddo
    ! enddo
  ...
  !  interpolate_volumefraction(:,snapshots,:)=snapmean_volumefraction(:,:)!jcp-revi
    ...
    call form_svd_opal_nowindows(interpolate_pressure, leftsvd_pressure, svdval_pressure, snapshots)
   ...
   ! call form_podstate_velocity(state(1,:,:),pod_state,leftsvd_velocity,leftsvd_pressure, snapmean_velocity, snapmean_pressure)
    ...
    ! call form_podstate_pressure_opal(state(1,:,:),pod_state_p, leftsvd_pressure,  snapmean_pressure)
    ...
    enddo !numphase 
     ...
     call calculate_pod_coef_all_timesteps_opal()
     print *, 'after calculate'
     training = .true.
     if (training) then
        call non_intrusive_opal_nowindows()
     else 
        call non_intrusive_opal_predict()
     endif
  ...
  end subroutine nonintrusive_snapshots_main_geothermal_opal 

   subroutine nonintrusive_snapshots_main_geothermal_opal_time_windows()
    !!< Matrices containing the snapshots for arpack 
    real, dimension(:,:,:,:,:), allocatable :: snapmatrix_velocity  
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field
    ...
    call put_time_windows_vtu_to_state(state)  
    call retrieve_nodesvalue_from_states(state,  snapmatrix_pressure ) 
    !allocate(snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
   ! allocate(snapmatrix_pressure(no_smolyak_nodes,p_nodes,snapshots,numphase)) 
    !!!!!!!form podbasis for interpolated snapshots
    !allocate(snapmean_velocity(u_nodes,dimen,numphase))
  ...
  !  interpolate_volumefraction(:,snapshots,:)=snapmean_volumefraction(:,:)!jcp-revi
    ...
    call cpu_time(cpu_basis_start)
    call form_svd_opal(interpolate_pressure, leftsvd_pressure, svdval_pressure, snapshots)
    call cpu_time(cpu_basis_end)
    print *, 'cpu_time_for_basis', cpu_basis_end-cpu_basis_start
    !!!stop 888
   ! call form_podstate_velocity(state(1,:,:),pod_state,leftsvd_velocity,leftsvd_pressure, snapmean_velocity, snapmean_pressure)
  ...
  end subroutine nonintrusive_snapshots_main_geothermal_opal_time_windows
  ...
  subroutine nonintrusive_snapshots_main()
   ...
   ! call read_input_states(state)
    ...
    call retrieve_nodesvalue_from_states2(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction) 
    !allocate(snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    !allocate(snapmatrix_pressure(no_smolyak_nodes,p_nodes,snapshots,numphase)) 
    !!!!!!!form podbasis for interpolated snapshots
    allocate(snapmean_velocity(u_nodes,dimen,numphase))
  ...
  subroutine calculate_pod_coef_all_timesteps_opal()    
  ...
  end subroutine calculate_pod_coef_all_timesteps_opal
  ...
  !  if (have_option("/reduced_model/Velocity_Interpolation")) then 
    ...
    ! visulisation
     ...
     do k=1, numphase 
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
        velocity_field => extract_vector_field(state(1,s,1), "Water::Velocity" )
        pressure_field => extract_scalar_field(state(1,s,1), "Water::Pressure" )
        volumefraction_field=> extract_scalar_field(state(1,s,1), "Water::MaterialVolumeFraction")!SolidConcentration" )  ! change
       ...
       call set_option('/simulation_name', trim(simulation_name)//'_final') 
        dump_no=s
              ...
              enddo
               pressure_field%val=(pressure_field%val(:)+ snapmean_pressure(:,k) -1)!+1000000
   ...
   subroutine non_intrusive_opal_nowindows()
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field,p2
    real, dimension(:), allocatable ::  pod_coef_orig,pod_coef_new,pod_coef_old
    integer :: i, ph,j,k,s,dump_no , d
    ...
    real, allocatable :: wm(:,:)
    real :: r0
    real , allocatable :: xd(:,:), xd_lq(:,:), pod_coef_all(:,:), smoylak_all(:,:),pod_coef_all_nodes(:,:,:), pod_coef_all_nowindows(:,:)
    real , allocatable :: xi(:,:)
    real , allocatable :: zd(:), fd(:)
    real , allocatable :: ze(:), temp_coef(:)
    real , allocatable :: zi(:), minin(:), maxin(:)
    ...
    integer :: m,ll,kk,u_nonods,l,nd, podnum,ni
   ! numphase=option_count("/material_phase")
    total_timestep=snapshots!total_dumps
  !  if (have_option("/reduced_model/Velocity_Interpolation")) then 
   ...
   ! endif
    m =  nsvd*numphase
    podnum =m   
     allocate(pod_coef_all_nodes(no_smolyak_nodes,total_timestep,podnum))    
     allocate(pod_coef_all(snapshots,podnum))  
     allocate(pod_coef_new(m)) 
     allocate(pod_coef_old(m)) 
     allocate(w(m))
      allocate(pod_coef_all_nowindows(snapshots,podnum)) 
     training = .true.
     if (training) then
        open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))
     else 
       open(unit=61,file="coef_pod_all_obv_normal_fault_predict")
     endif 
    ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))   
     !read(61,*)(((pod_coef_all_nodes(i,j,k),k=1,podnum),j=1,total_timestep), i=1, no_smolyak_nodes)
     read(61,*)((pod_coef_all(j,k),k=1,podnum),j=1,snapshots)     
     close(61)      
  ...
  ! do kk = 1, no_smolyak_nodes
  !  pod_coef_all(:,:)=pod_coef_all_nodes(kk,:,:)
   ...
   ENDDO 
    ...
    ! visulisation
     ...
     do k=1, numphase 
       ...
       ! velocity_field => extract_vector_field(state(1,s,1), "Water::Velocity" )
       ! pressure_field => extract_scalar_field(state(1,s,1), "phase1::Temperature" )
       pressure_field => extract_scalar_field(state(1,s,k), "Temperature" )
       ! volumefraction_field=> extract_scalar_field(state(1,s,1), "Water::MaterialVolumeFraction")!SolidConcentration" )  ! change
       pressure_field%val(:)=0
      ! velocity_field%val=0
      ! volumefraction_field%val(:)=0  
         ...
         ! print *, 'sizedimension', size(velocity_field%val,1), size(velocity_field%val,2)
           do i=1,nsvd   
               ! do d=1,dimen 
                  !velocity_field%val(d,:)=velocity_field%val(d,:)+pod_coef_all_obv(s,i+(d-1)*nsvd,k)*leftsvd_velocity(:,i,d,k)
               ! enddo
                !print *, 'no_smolyak_nodes, kk-1, s, i,k=', no_smolyak_nodes, kk-1, s, i,k               
              ! pressure_field%val(:)=pressure_field%val(:)+pod_coef_all_obv((kk-1)*total_timestep+s,i,k)*leftsvd_pressure(:,i,k)
             !   pressure_field%val(:)=pressure_field%val(:)+pod_coef_all_nowindows(s,i)*leftsvd_pressure(:,i,k)!pod_coef_all_obv  test
              !   pressure_field%val(:)=pressure_field%val(:)+pod_coef_all_obv(s,i,1)*leftsvd_pressure(:,i,k)
               !volumefraction_field%val(:)=volumefraction_field%val(:)+pod_coef_all_obv(s,i+(dimen+1)*nsvd,k)*leftsvd_volumefraction(:,i,k)  
               
                pressure_field%val(:)=pressure_field%val(:)+pod_coef_all(s,i)*leftsvd_pressure(:,i,k)
           enddo 
       ...
       call set_option('/simulation_name', trim(simulation_name)//'_final') 
        dump_no=s 
             ...
             !   velocity_field%val(d,:)=velocity_field%val(d,:)+snapmean_velocity(:,d,k)  !allocate(snapmean_velocity(u_nodes,dimen,numphase))
            !  enddo
            pressure_field%val= pressure_field%val(:) !+1000000
            ! volumefraction_field%val=(volumefraction_field%val+snapmean_volumefraction(:,k))!*800
        enddo !do k=1, numphase 
     ...
     ! call calculate_diagnostic_variables( state_show, exclude_nonrecalculated = .true. )
      ! call write_state(dump_no, state_show)
        ! call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVelocity", index=dump_no, model="VelocityMesh", state=pod_state(i,:,ph))
       !(d-1)*u_nodes+j
      ! d= int(s/no_smolyak_nodes)
      ! dump_no=MOD(s,no_smolyak_nodes)
      ! call vtk_write_state(filename=trim(simulation_name)//int2str(d), index=dump_no, model="Mesh", state=state(:,s,1))
       call vtk_write_state(filename=trim(simulation_name), index=s, model="Mesh", state=state(1,s,:))
     !  call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="PressureMesh", state=state(:,s,1))
       ! call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="Mesh", state=state(1,s,1))
      pod_coef_old=pod_coef_new ! iteration  
     ENDDO ! Do s=1, total_timestep
      !print *, 'pressure', pressure_field%val(2)
    ! enddo !do kk = 1, no_smolyak_nodes
     deallocate(pod_coef_new) 
    deallocate(pod_coef_old) 
     deallocate ( fi )
     deallocate ( xi ) 
     ...
     deallocate(xd)
     deallocate(w)
     deallocate(pod_coef_all_nodes)
    end subroutine non_intrusive_opal_nowindows

subroutine non_intrusive_opal()
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field,p2
    real, dimension(:), allocatable ::  pod_coef_orig,pod_coef_new,pod_coef_old
    integer :: i, ph,j,k,s,dump_no , d
  ...
  !  if (have_option("/reduced_model/Velocity_Interpolation")) then 
   ...
   ! endif
    ...
    !  open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))  !coef_pod_all_obv_normal_fault_predictset1
      open(unit=61,file="coef_pod_all_obv_normal_fault_predictset1")
     !read(61,*)(((pod_coef_all_nodes(i,j,k),k=1,podnum),j=1,total_timestep), i=1, no_smolyak_nodes)
     read(61,*)((pod_coef_all_nowindows(j,k),k=1,podnum),j=1,snapshots)     
     close(61)      
  ...
  !  pod_coef_all(:,:)=pod_coef_all_nodes(kk,:,:)
   ...
   ENDDO 
    ...
    deallocate(pod_coef_old) 
     ...
     deallocate(w)
     deallocate(pod_coef_all_nodes)
    end subroutine non_intrusive_opal

subroutine non_intrusive_opal_predict()
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field,p2
    real, dimension(:), allocatable ::  pod_coef_orig,pod_coef_new,pod_coef_old
    integer :: i, ph,j,k,s,dump_no , d
    ...
    real, allocatable :: wm(:,:)
    real :: r0
    real , allocatable :: xd(:,:), xd_lq(:,:), pod_coef_all(:,:), smoylak_all(:,:),pod_coef_all_nodes(:,:,:)
    real , allocatable :: xi(:,:)
    real , allocatable :: zd(:), fd(:)
    real , allocatable :: ze(:), temp_coef(:)
    real , allocatable :: zi(:), minin(:), maxin(:)
    ...
    real ( kind = 8 ), allocatable :: fi(:) 
    real :: maxall,minall
    integer :: m,ll,kk,u_nonods,l,nd, podnum,ni,number_predict_sets
   ! numphase=option_count("/material_phase")
    total_timestep=total_dumps !30
    number_predict_sets = 1
  !  if (have_option("/reduced_model/Velocity_Interpolation")) then 
   ...
   ! endif
    m =  nsvd*numphase
    podnum =m   
     allocate(pod_coef_all_nodes(number_predict_sets,total_timestep,podnum))    
     allocate(pod_coef_all(total_timestep,podnum))  
     allocate(pod_coef_new(m)) 
     allocate(pod_coef_old(m)) 
    allocate(w(m))
     
     open(unit=61,file="coef_pod_all_obv_normal_fault_predict50-50")
     !open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))  
     read(61,*)(((pod_coef_all_nodes(i,j,k),k=1,podnum),j=1,total_timestep), i=1, number_predict_sets)     
     close(61)      
   ...
   ENDDO 
    ...
    ! visulisation
     ...
     do k=1, numphase 
       ...
       ! velocity_field => extract_vector_field(state(1,s,1), "Water::Velocity" )
        pressure_field => extract_scalar_field(state(kk,s,1), "phase1::Temperature" )
       ! volumefraction_field=> extract_scalar_field(state(1,s,1), "Water::MaterialVolumeFraction")!SolidConcentration" )  ! change
       pressure_field%val(:)=0
      ! velocity_field%val=0
      ! volumefraction_field%val(:)=0  
     ...
     ! call calculate_diagnostic_variables( state_show, exclude_nonrecalculated = .true. )
      ...
      ! dump_no=MOD(s,no_smolyak_nodes)
      ! call vtk_write_state(filename=trim(simulation_name)//int2str(d), index=dump_no, model="Mesh", state=state(:,s,1))
       call vtk_write_state(filename=trim(simulation_name)//"_predict"//int2str(kk-1), index=dump_no, model="Mesh", state=state(kk,s,:))
     !  call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="PressureMesh", state=state(:,s,1))
       ! call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="Mesh", state=state(1,s,1))
      pod_coef_old=pod_coef_new ! iteration  
     ENDDO ! Do s=1, total_timestep
    ...
    deallocate(pod_coef_old) 
     ...
     deallocate(w)
     deallocate(pod_coef_all_nodes)
    end subroutine non_intrusive_opal_predict
 ...
 !!!  for case has the same mesh like P1_P1
     ...
     do i=1, total_dumps
         ! if(numphase .eq. 1) then
           !  write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
              write(filename, '(a, i0, a)')  'dun_water_collapse_', (i)*dump_sampling_period,".vtu" 
             call vtk_read_state(filename, state(k,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_CoordinateMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
       ...
       volumefraction=extract_scalar_field(state_p(k,i,j),"MaterialVolumeFraction")
        call insert(state(k,i,j),  volumefraction, name="MaterialVolumeFraction")
  ...
  !  stop 12
     ...
     subroutine put_time_windows_vtu_to_state(state)
    !!< Read the input states from the vtu dumps of the forward run. ! different mesh such as P1_dg_P2
    type(state_type), intent(out), dimension(:,:,:), allocatable :: state
    !type(state_type), intent(out), dimension(:,:,:), allocatable :: state_p
    character(len=1024) :: filename
    ...
    integer :: w 
    character(len=1024) :: phase_name
      type(scalar_field), pointer :: pressure1!, volumefraction  
    call get_option('/reduced_model/pod_basis_formation/dump_sampling_period',dump_sampling_period)
    call get_option('/geometry/quadrature/degree', quadrature_degree)
   ...
   ! numphase=option_count("/material_phase")
    allocate(state(1,windows*2*no_smolyak_nodes,numphase))
    allocate(state_p(no_smolyak_nodes,total_dumps,numphase))
    ...
    do w =1, windows
     ...
     !do i=w, w+1
         ! if(numphase .eq. 1) then
             !write(filename, '(a, i0, a)')  trim(simulation_name)//'_', (i)*dump_sampling_period,".vtu" 
             write(filename, '(a, i0,a,i0, a, i0,a)')  'Window_', w-1, '_Ensemble_Temperature_', k-1, '/'//trim(simulation_name)//'_', (w-1+j-1)*dump_sampling_period,".vtu" 
             call vtk_read_state(filename, state(1, ((k-1)*windows+w-1)*2+j,1), quadrature_degree)
             print *,'k,w,j, filename=(k-1)*windows+w-1)*2+j', k,w,j, ((k-1)*windows+w-1)*2+j, filename !三维数组维展a[M][N][P],下标为a[m][n][p]，则一维表达式为a[ (m*N+n)*P+p]
             ! write(filename, '(a, i0,a,i0, a, i0,a)')  'Window_', w-1, '_Ensemble_Temperature_', k-1, '/'//trim(simulation_name)//'_', (w)*dump_sampling_period,".vtu" 
            ! call vtk_read_state(filename, state(1,windows+w,1), quadrature_degree)
           !  print *,'filename=', filename  
            ! write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_CoordinateMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
            ! call vtk_read_state(filename, state_p(k,i,j), quadrature_degree) 
            !  print *,'filename=', filename 
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    !velocity => extract_vector_field(state(1,1,1), "Water::Velocity")
    !pressure => extract_scalar_field(state(1,1,1), "phase1::Temperature") !(state_p(1,1,1), "Pressure") 
    ! call print_state(state(1,1,1)) size of state is not equvilent. no scalar fields named temperature in the field.
    ! pressure1 => extract_scalar_field(state(1,1,1), "Temperature")
     print *, 'put_time_windows_vtu_to_state finished--------------------------------------'!, pressure1%val(:)
    !call print_state(state(1,1,1))
   end subroutine put_time_windows_vtu_to_state

    subroutine put_vtu_to_state(state)
    !!< Read the input states from the vtu dumps of the forward run. ! different mesh such as P1_dg_P2
    type(state_type), intent(out), dimension(:,:,:), allocatable :: state
    !type(state_type), intent(out), dimension(:,:,:), allocatable :: state_p
    character(len=1024) :: filename
    ...
    !substract gyre_0.vtu
    !total_dumps=30!count_dumps(dump_sampling_period)
   ! numphase=option_count("/material_phase")
    allocate(state(1,total_dumps*no_smolyak_nodes,numphase))
    allocate(state_p(no_smolyak_nodes,total_dumps,numphase))
    ...
    do k=1, no_smolyak_nodes
     ...
     do i=1, total_dumps
         ! if(numphase .eq. 1) then
             !write(filename, '(a, i0, a)')  trim(simulation_name)//'_', (i)*dump_sampling_period,".vtu" 
             write(filename, '(a, i0, a, i0,a)')  'Window_0_Ensemble_Temperature_', k-1, '/'//trim(simulation_name)//'_', (i-1)*dump_sampling_period,".vtu" 
             call vtk_read_state(filename, state(1,total_dumps*(k-1)+i,j), quadrature_degree)
             print *,'filename=', filename  
            ! write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_CoordinateMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
            ! call vtk_read_state(filename, state_p(k,i,j), quadrature_degree) 
            !  print *,'filename=', filename
    ...
    enddo 
    enddo
   end subroutine put_vtu_to_state
  ...
  !  numphase=option_count("/material_phase")
   ...
   subroutine retrieve_nodesvalue_from_states2(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    velocity => extract_vector_field(state(1,1,1), "Water::Velocity")
    pressure => extract_scalar_field(state(1,1,1), "Water::Pressure") !(state_p(1,1,1), "Pressure") 
    volumefraction=> extract_scalar_field(state(1,1,1), "Water::MaterialVolumeFraction")!(state_p(1,1,1), "SolidConcentration")! change
    dimen=velocity%dim
   ...
   do p = 1, numphase
    do i = 1, snapshots
       velocity => extract_vector_field(state(k,i,p), "Water::Velocity")
       ...
       pressure => extract_scalar_field(state(k,i,1), "Water::Pressure")
       print *, 'p_nodes=node_count(pressure)', p_nodes
       volumefraction=> extract_scalar_field(state(k,i,p), "Water::MaterialVolumeFraction")! change
       do d = 1, dimen 
  ...
  end subroutine retrieve_nodesvalue_from_states2

  subroutine retrieve_nodesvalue_from_states(state, snapmatrix_pressure)  
    type(state_type), intent(in), dimension(:,:,:) :: state
    !real, dimension(:,:,:,:,:), allocatable :: snapmatrix_velocity
    real, dimension(:,:,:,:), allocatable :: snapmatrix_pressure!,snapmatrix_volumefraction 
    ! integer :: numphase
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    ...
    !volumefraction=> extract_scalar_field(state(1,1,1), "Water::MaterialVolumeFraction")!(state_p(1,1,1), "SolidConcentration")! change
    !dimen=velocity%dim
    p_nodes=node_count(pressure1)
    !u_nodes=node_count(velocity)
    !v_nodes=node_count(volumefraction)
    snapshots=size(state(1,:,1))
    !allocate(snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    allocate(snapmatrix_pressure(1,p_nodes,snapshots,numphase)) 
    !allocate(snapmatrix_volumefraction(no_smolyak_nodes,v_nodes,snapshots,numphase))
   ! print *, 'p_nodes=node_count(pressure)', p_nodes
    !print *, 'u_nodes=node_count(velocity)', u_nodes, size(velocity%val,2)
    !snapmatrix_velocity=0.0
  ...
  ! do k=1, no_smolyak_nodes
   do p = 1, numphase
    do i = 1, snapshots
       ...
       !volumefraction=> extract_scalar_field(state_p(k,i,p), "SolidConcentration")
       !print *, 'u_nodes=node_count(velocity---iii)', u_nodes, size(velocity%val,2)
       !pressure => extract_scalar_field(state(k,i,1), "phase1::Temperature")
       !print *, 'k,i=============', k,i
       pressure1 => extract_scalar_field(state(1,i,1), "Temperature")
        !print *, 'k,i===========================after', k,i
       !print *, 'p_nodes=node_count(pressure)', p_nodes
       !volumefraction=> extract_scalar_field(state(k,i,p), "Water::MaterialVolumeFraction")! change
       !do d = 1, dimen 
         !  snapmatrix_velocity(k,:,i,d,p)=velocity%val(d,:)
      ! end do 
           snapmatrix_pressure(1,:,i,p)=pressure1%val
           !snapmatrix_volumefraction(k,:,i,p)=volumefraction%val!(p,:)
   ! end do
   enddo 
   enddo !end numphase  
    print *, 'retrieve_nodesvalue_from_states finished'
  end subroutine retrieve_nodesvalue_from_states

subroutine retrieve_nodesvalue_from_states_nowindows(state, snapmatrix_pressure)  
    type(state_type), intent(in), dimension(:,:,:) :: state
    !real, dimension(:,:,:,:,:), allocatable :: snapmatrix_velocity
    real, dimension(:,:,:,:), allocatable :: snapmatrix_pressure!,snapmatrix_volumefraction 
    ! integer :: numphase
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    ...
    snapshots=size(state(1,:,1))
    !allocate(snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    allocate(snapmatrix_pressure(1,p_nodes,snapshots*no_smolyak_nodes,numphase)) 
    !allocate(snapmatrix_volumefraction(no_smolyak_nodes,v_nodes,snapshots,numphase))
   ! print *, 'p_nodes=node_count(pressure)', p_nodes
    !print *, 'u_nodes=node_count(velocity)', u_nodes, size(velocity%val,2)
    !snapmatrix_velocity=0.0
  ...
  ! do k=1, no_smolyak_nodes
   ...
   enddo !end numphase  
    print *, 'retrieve_nodesvalue_from_states finished'
  end subroutine retrieve_nodesvalue_from_states_nowindows
  ...
  subroutine form_svd_opal(snapmatrix_pressure, leftsvd_pressure, svdval_pressure, snapshots)
   ...
   ! end do
    ...
    enddo !numphase
      ...
      if((i .eq. 1)) then                       
                   	...
                   	open(11,file="leftsvd_pressure"//"_"//trim(simulation_name), position='append',ACTION='WRITE') 
                          write(11,*)(leftsvd_pressure(:,i,1))   
                   	close(11)
                     endif
      enddo
   
  end subroutine form_svd_opal 

subroutine form_svd_opal_nowindows(snapmatrix_pressure, leftsvd_pressure, svdval_pressure, snapshots)
  ...
  end subroutine form_svd_opal_nowindows
 ...
 subroutine form_podstate_velocity(state, pod_state, leftsvd_u, leftsvd_p, snapmean_u, snapmean_p)
    ...
    do i = 1,POD_num
       ...
       ! print *, 'aaaaaaaaaaaaaaaaaaaaaaaa'
        pod_xmesh => extract_mesh(state(1,p), "P0Mesh")
        print *, 'form_podstate_velocity',i
        call print_state(state(1,1))
        pod_umesh =>extract_mesh(state(1,p), "P0Mesh")
       ...
       velocity => extract_vector_field(state(1,p), "Water::Velocity")
...
end program non_intrusive_vtu_geothermal_opal
