   ...
   allocate (xd(podnum,nd)) 
    ...
    ! visulisation
     ...
     do k=1, numphase 
       ...
       volumefraction_field=> extract_scalar_field(state(1,s,k), "Temperature" )  
     ...
     call write_state(dump_no, state_show)
        call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="VelocityMesh", state=state_show(:))
