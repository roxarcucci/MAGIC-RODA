...
#include "finclude/petsc.h"
  ...
  integer :: no_smolyak_nodes, m, sparse_max,snapshots, u_nodes, statei,p_nodes,v_nodes, snapshots_each
  ...
  type(state_type), dimension(:,:,:), allocatable :: pod_state_p, pod_state_v
    real, dimension(:,:,:,:), allocatable ::  interpolate_velocity, interpolate_velocity_temp
  ...
  no_smolyak_nodes=5 !no use in this program, but keep it for consitency with interpolate_snapshots_nonintruve_material_poeperty.
  ! m = 4
  ! sparse_max =1
  ! call get_option('/reduced_model/smolyak_order', sparse_max) 
  call nonintrusive_snapshots_main() 
 ! call nonintrusive_variable_parameters_main()
...
contains 
  ...
  subroutine nonintrusive_snapshots_main()
    ...
    integer :: i, dump_no, d, j,k ,m,i2,i3,ph,p,jj
    integer :: stat       
    ...
    allocate(interpolate_velocity(u_nodes,snapshots+1,dimen,numphase)) !need more (snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    allocate(interpolate_pressure(p_nodes,snapshots+1,numphase)) 
    allocate(interpolate_volumefraction(p_nodes,snapshots+1,numphase))
    print *, 'snapshots', snapshots, size(interpolate_velocity,2)
     snapmean_velocity=0
     snapmean_pressure=0
    ...
    enddo !end numphase  
         ...
         interpolate_velocity(:,snapshots,:,:)=snapmean_velocity(:,:,:) !jcp-revi! add the mean to the last column of the snapshot matrix
         interpolate_pressure(:,snapshots,:)=snapmean_pressure(:,:) !jcp-revi
        interpolate_volumefraction(:,snapshots,:)=snapmean_volumefraction(:,:)!jcp-revi
    ...
    do i=1,nsvd 
        dump_no=i 
       call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVelocity_meanend", index=dump_no, model="VelocityMesh", state=pod_state(i,:,ph))
       call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisPressure_meanend", index=dump_no, model="PressureMesh", state=pod_state_p(i,:,ph))
       call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVolumefraction_meanend", index=dump_no, model="VolumefractionMesh", state=pod_state_v(i,:,ph))
  ...
  subroutine nonintrusive_variable_parameters_main()
    !!< Matrices containing the snapshots for arpack
    
    real, dimension(:,:,:,:,:), allocatable :: snapmatrix_velocity  
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field
    integer :: i, dump_no, d, j,k ,m,i2,i3,ph,p,jj,s
    integer :: stat       
   ...
   ! call print_state(state_show(1))    
    !call read_input_states(state)
     call put_vtu_to_states(state,state_p)  
    !call put_all_vtu_to_states_blast(state,state_p)  
    call retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction) 
    ...
    allocate(interpolate_velocity(u_nodes,snapshots*no_smolyak_nodes,dimen,numphase)) !need more
    allocate(interpolate_pressure(p_nodes,snapshots*no_smolyak_nodes,numphase)) 
    allocate(interpolate_volumefraction(p_nodes,snapshots*no_smolyak_nodes,numphase))
     print *, 'snapshots', snapshots, size(interpolate_velocity,2)
     snapmean_velocity=0
     snapmean_pressure=0
     snapmean_volumefraction=0 
   
      do jj=1, no_smolyak_nodes
       do i=1, snapshots
        interpolate_velocity(:,((jj-1)*snapshots)+i,:,:)=snapmatrix_velocity(jj,:,i,:,:) 
        interpolate_pressure(:,((jj-1)*snapshots)+i,:)=snapmatrix_pressure(jj,:,i,:)!-1000000
        interpolate_volumefraction(:,((jj-1)*snapshots)+i,:)=snapmatrix_volumefraction(jj,:,i,:)!/800
     enddo
     enddo
     do p = 1, numphase
     do i=1, snapshots
    ...
    enddo !end numphase  
     ...
     ! interpolate_volumefraction(:,snapshots,:)=snapmean_volumefraction(:,:)!jcp-revi
    call form_svd_mp(interpolate_velocity, interpolate_pressure, interpolate_volumefraction,&
         & leftsvd_velocity, leftsvd_pressure, leftsvd_volumefraction, svdval_velocity, svdval_pressure, svdval_volumefraction, snapshots*no_smolyak_nodes)
    ...
    ! call calculate_pod_coef_all_timesteps()
       ...
       allocate(pod_coef_all_obv(snapshots,(dimen+2)*nsvd,numphase)) 
         do jj=1, no_smolyak_nodes
          ! print *, size(interpolate_velocity,1), size(snapmatrix_velocity,2)
           !interpolate_velocity(:,:,:,:)=snapmatrix_velocity(jj,:,:,:,:) 
           !interpolate_pressure=snapmatrix_pressure(jj,:,:,:) 
           !interpolate_volumefraction=snapmatrix_volumefraction(jj,:,:,:) 
            do s=1, snapshots
                 do k=1, numphase 
                   do i=1,size(POD_state,1) 
                             do j=1,dimen !velocity%dim 
                            ! pod_coef_all_obv(s,i+nsvd*(j-1),k)=dot_product(leftsvd_velocity(:,i,j,k),(interpolate_velocity(:,s,j,k)))   
                              pod_coef_all_obv(s,i+nsvd*(j-1),k)=dot_product(leftsvd_velocity(:,i,j,k),(snapmatrix_velocity(jj,:,s,j,k) ))   
                                                                     
                             enddo 
                            ! pod_coef_all_obv(s,i+nsvd*dimen,k)=dot_product(leftsvd_pressure(:,i,k),(interpolate_pressure(:,s,k)))                             
                            ! pod_coef_all_obv(s,i+nsvd*(dimen+1),k)=dot_product(leftsvd_volumefraction(:,i,k),(interpolate_volumefraction(:,s,k)))
                             pod_coef_all_obv(s,i+nsvd*dimen,k)=dot_product(leftsvd_pressure(:,i,k),(snapmatrix_pressure(jj,:,s,k)))                             
                             pod_coef_all_obv(s,i+nsvd*(dimen+1),k)=dot_product(leftsvd_volumefraction(:,i,k),(snapmatrix_volumefraction(jj,:,s,k)))
                     enddo   
                       if((s .eq. 1).and.(k.eq.1)) then                       
                    	open(11,file="coef_pod_all_obv"//"_"//int2str(jj))  
                        write(11,*)(pod_coef_all_obv(s,:,k)) 
                   	close(11) 
                    else 
                   	open(11,file="coef_pod_all_obv"//"_"//int2str(jj), position='append',ACTION='WRITE') 
                        write(11,*)(pod_coef_all_obv(s,:,k)) 
                   	close(11)
                     endif
  ...
  end subroutine nonintrusive_variable_parameters_main 


 subroutine calculate_pod_coef_for_each_simulation()    
                 ...
                 do k=1, numphase 
                   do i=1,size(POD_state,1) 
                             ...
                             pod_coef_all_obv(s,i+nsvd*(j-1),k)=&
                                        dot_product(leftsvd_velocity(:,i,j,k),(interpolate_velocity(:,s,j,k)))   
                                                                    
                             enddo 
                             pod_coef_all_obv(s,i+nsvd*dimen,k)= &
                                        dot_product(leftsvd_pressure(:,i,k),(interpolate_pressure(:,s,k)))
                             
                             pod_coef_all_obv(s,i+nsvd*(dimen+1),k)= &
                                        dot_product(leftsvd_volumefraction(:,i,k),(interpolate_volumefraction(:,s,k)))
                     enddo   
                       if((s .eq. 1).and.(k.eq.1)) then                       
                    	open(11,file="coef_pod_all_obv"//"_"//int2str(j))  
                            write(11,*)(pod_coef_all_obv(s,:,k)) 
                             
                   	close(11) 
                    else 
                   	open(11,file="coef_pod_all_obv"//"_"//int2str(j), position='append',ACTION='WRITE') 
                         write(11,*)(pod_coef_all_obv(s,:,k))  
  ...
  end subroutine calculate_pod_coef_for_each_simulation
  ...
  subroutine non_intrusive_state_show()
    type(vector_field), pointer :: velocity_field
   ...
   if (have_option("/reduced_model/Velocity_Interpolation")) then 
     ...
     ! open(unit=61,file="pod_coef_variable_out")   ! for variable parameter input open
      read(61,*)((pod_coef_all(j,k),k=1,podnum),j=1,total_timestep)
      close(61)      
   else
      open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name)) ! pod_coef_variable_out 
     ! open(unit=61,file="pod_coef_variable_out") !coef_pod_all_obv_1   ! for variable parameter input
   ...
   allocate (xd(podnum,nd)) 
    ...
    ! visulisation
     ...
     do k=1, numphase 
        !velocity_field => extract_vector_field(state_show(k), "Velocity" )
       ! pressure_field => extract_scalar_field(state_show(k), "Pressure" )
       ! volumefraction_field=> extract_scalar_field(state_show(k), "Density")!SolidConcentration" )  ! change
         velocity_field => extract_vector_field(state(1,s,1), "Velocity" )
         pressure_field => extract_scalar_field(state(1,s,1), "Pressure" )
         volumefraction_field=> extract_scalar_field(state(1,s,1), "Density")!SolidConcentration" )  ! change
       ...
       volumefraction_field%val(:)=0  
          ...
          print *, 'sizedimension', size(velocity_field%val,1), size(velocity_field%val,2)
           do i=1,nsvd   
                ...
                ! velocity_field%val(d,:)=velocity_field%val(d,:)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k) !?!variable inputs jcp-first
                 !
                enddo
               ...
               !  pressure_field%val(:)=pressure_field%val(:)+pod_coef_all(s,i+dimen*nsvd)*leftsvd_pressure(:,i,k)  !variable inputs jcp-first
               ! volumefraction_field%val(:)=volumefraction_field%val(:)+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_volumefraction(:,i,k)  !variable inputs jcp-first
           enddo 
        !  velocity_field%val=0
        !  pressure_field%val=0
       ...
       call set_option('/simulation_name', trim(simulation_name)//'_final') 
        dump_no=s
              ...
              !   call insert(state_show(k), pressure_field, "Pressure")
               ...
               !  call insert(state(1,s,1), pressure_field, "Pressure")
                !  call insert(state(1,s,1), volumefraction_field, "Volume_fraction")
                !  call insert(state(1,s,1), velocity_field, "Velocity")
    ...
    deallocate(pod_coef_old) 
     ...
     deallocate(xd)
     deallocate(w)
    end subroutine non_intrusive_state_show
  ...
  subroutine non_intrusive()
   ...
   if (have_option("/reduced_model/Velocity_Interpolation")) then 
     ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name)) 
      open(unit=61,file="pod_coef_variable_out")   
      read(61,*)((pod_coef_all(j,k),k=1,podnum),j=1,total_timestep)
      close(61)      
   else
    ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))  pod_coef_variable_out 
      open(unit=61,file="pod_coef_variable_out") !coef_pod_all_obv_1
    ! open(unit=61,file="coef_pod_all_obv_1") !
     read(61,*)((pod_coef_all(j,k),k=1,podnum),j=1,total_timestep)      
     close(61)      
    endif
    ...
    ! visulisation
       ...
       volumefraction_field%val(:)=0  
          ...
          print *, 'sizedimension', size(velocity_field%val,1), size(velocity_field%val,2)
           do i=1,nsvd   
              ...
              !  volumefraction_field%val(:)=volumefraction_field%val(:)+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_volumefraction(:,i,k)  
           enddo 
          velocity_field%val=0
          pressure_field%val=0
     ...
     ! call calculate_diagnostic_variables( state_show, exclude_nonrecalculated = .true. )
      ! call write_state(dump_no, state_show)
        ! call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVelocity", index=dump_no, model="VelocityMesh", state=pod_state(i,:,ph))
       call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="Mesh", state=state(:,s,1))
       call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="PressureMesh", state=state(:,s,1)) ! checkpoint
       ! call vtk_write_state(filename=trim(simulation_name), index=dump_no, model="Mesh", state=state(1,s,1))
      pod_coef_old=pod_coef_new ! iteration  
    ENDDO ! Do s=1, total_timestep
     deallocate(pod_coef_new) 
 ...
 subroutine interpolate_coef()  
  implicit none    
  integer ( kind = 4 ) h 
  integer ( kind = 4 ) nd 
  integer ( kind = 4 ) ni, j, k,jj 
  real, allocatable :: wm(:,:)
  ...
  real ( kind = 8 ), allocatable :: fi(:) 
  real :: r0
  real, dimension(:,:,:), allocatable :: pod_coef_variable 
  real, dimension(:,:), allocatable :: pod_coef_variable_out 
  integer i, ii, total, total_timestep, num_pod,  kk, smolyak_total_points, u_nodesi,snapshotsi,dimi,numphasei, podnum 
  total_timestep=total_dumps
  m = 1
  podnum = (dimen+2)*nsvd*numphase
  ni = 1
   nd = no_smolyak_nodes
   allocate ( xi(1:m,1:ni) )
   allocate ( fi(1:ni) ) 
   ...
   allocate (wm(nd,1))
   allocate (xd(m,nd))
   allocate(pod_coef_variable(5,total_timestep,podnum))
   allocate(pod_coef_variable_out(total_timestep,podnum))
  ! call calculate_pod_coef_for_each_simulation()
   simulation_name="blasting_cubic_140p_POD"
  
  do jj=1, nd 
   open(unit=61,file="coef_pod_all_obv"//"_"//int2str(jj) )   
   read(61,*)((pod_coef_variable(jj,j,k),k=1,podnum),j=1,total_timestep)
   close(61) 
  enddo

  ! simulation_name="blasting_cubic_075p_POD"
  ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))   
 !  read(61,*)((pod_coef_variable(2,j,k),k=1,podnum),j=1,total_timestep)
  ! close(61) 

  ! simulation_name="blasting_cubic_090p_POD"
  ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))   
  ! read(61,*)((pod_coef_variable(3,j,k),k=1,podnum),j=1,total_timestep)
  ! close(61) 
   
   
  ! simulation_name="blasting_cubic_130p_POD"
  ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))   
 !  read(61,*)((pod_coef_variable(4,j,k),k=1,podnum),j=1,total_timestep)
  ! close(61) 
   
  ! simulation_name="blasting_cubic_140p_POD"
  ! open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name))   
  ! read(61,*)((pod_coef_variable(5,j,k),k=1,podnum),j=1,total_timestep)
  ! close(61) 
    ...
    Do k=1, podnum
     Do j=1, total_timestep  
     xd(1,1)=50 
      fd(1)=pod_coef_variable(1,j,k) 
     xd(1,2)=75 
      fd(2)=pod_coef_variable(2,j,k) 
     xd(1,3)=90 
      fd(3)=pod_coef_variable(3,j,k) 
     xd(1,4)= 130 
       fd(4)=pod_coef_variable(4,j,k) 
     xd(1,5)= 140  
       fd(5)=pod_coef_variable(5,j,k)
      
        
     ! print * ,'xd',xd
     ! r0 = ( 9.869233E-013 - 3.2897443e-13 ) / real ( nd, kind = 8 )
     r0 = ( 140 - 50 ) / real ( nd, kind = 8 )
     !r0=1
      call rbf_weight ( m, nd, xd, r0, phi4, fd, wm(:,1) )
     ! enddo 
       !  xi(1,1)=450!0.00000000000108561563 !!0.000000000000453984718!0.000000000000049346165  ! 0.0000000000004687885675
       ! xi(2,1)= 1250!0.00000000000108561563!0.000000000000355292388!0.000000000000049346165 ! 0.000000000000740192475
      xi(1,1)= 90   
      call rbf_interp_nd ( m, nd, xd, r0, phi4, wm(:,1), ni, xi, fi ) 
      !fi(1)=fi(1)*0.0000000000009869233/1000
      pod_coef_variable_out(j,k)=fi(1)
     ! print *, 'output', fi(1)
   enddo  
  enddo  
   
   open(12,file="pod_coef_variable_out")  
   ! write(12,*)(pod_coef_variable_out(1:total_timestep,1:podnum))  
   do j=1, total_timestep 
       write(12,*)  pod_coef_variable_out(j,1:podnum)
   enddo 
   close(12) 

 ! allocate(pod_coef_all_obv(total_timestep,podnum,numphase))
 ! pod_coef_all_obv(:,:,1)=pod_coef_variable_out(:,:)

  deallocate(pod_coef_variable_out)
  deallocate(pod_coef_variable)
  deallocate (fi)
  deallocate (xi) 
  deallocate(fd) 
  deallocate(wm)
 ...
 !!!  for case has the same mesh like P1_P1
  ...
  subroutine put_vtu_to_states(state,state_p)
    ...
    do k=1, no_smolyak_nodes
     ...
     do i=1, total_dumps
       ...
       volumefraction=extract_scalar_field(state_p(k,i,j),"Density")
        call insert(state(k,i,j),  volumefraction, name="Density")
  ...
  subroutine put_all_vtu_to_states_blast(state,state_p)
    !!< Read the input states from the vtu dumps of the forward run. ! different mesh such as P1_dg_P2
    type(state_type), intent(out), dimension(:,:,:), allocatable :: state
    type(state_type), intent(out), dimension(:,:,:), allocatable :: state_p
    character(len=1024) :: filename
    ...
    !substract gyre_0.vtu
    total_dumps=count_dumps(dump_sampling_period)
    snapshots_each=total_dumps!/no_smolyak_nodes
    numphase=option_count("/material_phase")
    allocate(state(no_smolyak_nodes,snapshots_each,numphase))
    allocate(state_p(no_smolyak_nodes,snapshots_each,numphase))
    
    !    vtu--->state
   ! do k=1, no_smolyak_nodes
   simulation_name = "blasting_cubic_050p"
   do j=1, numphase       
     do i=1, snapshots_each         
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state(1,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'PressureMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state_p(1,i,j), quadrature_degree) 
             print *,'filename=', filename               
          pressuremesh =extract_mesh(state_p(1,i,j), "Mesh") 
        call insert(state(1,i,j), pressuremesh, name="PressureMesh")       
        pressure=extract_scalar_field(state_p(1,i,j),"Pressure")
       call insert(state(1,i,j),  pressure, name="Pressure")    
       volumefraction=extract_scalar_field(state_p(1,i,j),"Density")
       call insert(state(1,i,j),  volumefraction, name="Density")
       ! call insert(state(k,i,j), pressure_field, "Pressure")   change 5 places if changing one field
    enddo
    enddo  
    ! enddo
    simulation_name = "blasting_cubic_075p"
   do j=1, numphase       
     do i=1, snapshots_each         
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state(2,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'PressureMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state_p(2,i,j), quadrature_degree) 
             print *,'filename=', filename               
          pressuremesh =extract_mesh(state_p(2,i,j), "Mesh") 
        call insert(state(2,i,j), pressuremesh, name="PressureMesh")       
        pressure=extract_scalar_field(state_p(2,i,j),"Pressure")
       call insert(state(2,i,j),  pressure, name="Pressure")    
       volumefraction=extract_scalar_field(state_p(2,i,j),"Density")
       call insert(state(2,i,j),  volumefraction, name="Density")
       ! call insert(state(k,i,j), pressure_field, "Pressure")   change 5 places if changing one field
    enddo
    enddo 

     simulation_name = "blasting_cubic_090p"
   do j=1, numphase       
     do i=1, snapshots_each         
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state(3,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'PressureMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state_p(3,i,j), quadrature_degree) 
             print *,'filename=', filename               
          pressuremesh =extract_mesh(state_p(3,i,j), "Mesh") 
        call insert(state(3,i,j), pressuremesh, name="PressureMesh")       
        pressure=extract_scalar_field(state_p(3,i,j),"Pressure")
       call insert(state(3,i,j),  pressure, name="Pressure")    
       volumefraction=extract_scalar_field(state_p(3,i,j),"Density")
       call insert(state(3,i,j),  volumefraction, name="Density")
       ! call insert(state(k,i,j), pressure_field, "Pressure")   change 5 places if changing one field
    enddo
    enddo
   

     simulation_name = "blasting_cubic_130p"
   do j=1, numphase       
     do i=1, snapshots_each         
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state(4,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'PressureMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state_p(4,i,j), quadrature_degree) 
             print *,'filename=', filename               
          pressuremesh =extract_mesh(state_p(4,i,j), "Mesh") 
        call insert(state(4,i,j), pressuremesh, name="PressureMesh")       
        pressure=extract_scalar_field(state_p(4,i,j),"Pressure")
       call insert(state(4,i,j),  pressure, name="Pressure")    
       volumefraction=extract_scalar_field(state_p(4,i,j),"Density")
       call insert(state(4,i,j),  volumefraction, name="Density")
       ! call insert(state(k,i,j), pressure_field, "Pressure")   change 5 places if changing one field
    enddo
    enddo

      simulation_name = "blasting_cubic_140p"
   do j=1, numphase       
     do i=1, snapshots_each         
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state(5,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'PressureMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state_p(5,i,j), quadrature_degree) 
             print *,'filename=', filename               
          pressuremesh =extract_mesh(state_p(5,i,j), "Mesh") 
        call insert(state(5,i,j), pressuremesh, name="PressureMesh")       
        pressure=extract_scalar_field(state_p(5,i,j),"Pressure")
       call insert(state(5,i,j),  pressure, name="Pressure")    
       volumefraction=extract_scalar_field(state_p(5,i,j),"Density")
       call insert(state(5,i,j),  volumefraction, name="Density")
       ! call insert(state(k,i,j), pressure_field, "Pressure")   change 5 places if changing one field
    enddo
    enddo
   end subroutine put_all_vtu_to_states_blast
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    ...
    volumefraction=> extract_scalar_field(state(1,1,1), "Density")!(state_p(1,1,1), "SolidConcentration")! change
    dimen=velocity%dim
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       !volumefraction=> extract_scalar_field(state_p(k,i,p), "SolidConcentration")
       !print *, 'u_nodes=node_count(velocity---iii)', u_nodes, size(velocity%val,2)
       pressure => extract_scalar_field(state(k,i,1), "Pressure")
       !print *, 'p_nodes=node_count(pressure)', p_nodes
       volumefraction=> extract_scalar_field(state(k,i,p), "Density")! change
       do d = 1, dimen 
 ...
 subroutine form_podstate_velocity(state, pod_state, leftsvd_u, leftsvd_p, snapmean_u, snapmean_p)
    ...
    do i = 1,POD_num
      ...
      ! pod_mesh => extract_mesh(state(1), "Mesh")
       ...
       ! pod_pmesh =>extract_mesh(state_p(1,p), "Mesh") 
        pod_pmesh =>extract_mesh(state_p(1,p), "PressureMesh") !checkpoint
  ...
  subroutine form_podstate_volumefraction(state_p,pod_state_v,leftsvd_v, snapmean_v)
    ...
    print *, 'form_podstate_volumefraction'
    do p=1,numphase
    do i = 1,POD_num
      ...
      ! pod_mesh => extract_mesh(state(1), "Mesh")
       ...
       all_meshes_same = .true. 
       pod_xmesh => extract_mesh(state_p(1,p), "Mesh") 
       pod_pmesh =>extract_mesh(state_p(1,p), "PressureMesh")  !change to pressuremesh from mesh, checkpoint
      ! pod_pmesh =>extract_mesh(state_p(1,p), "Mesh")
       pod_positions => extract_vector_field(state_p(1,p), "Coordinate") 
