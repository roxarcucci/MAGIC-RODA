...
!    call set_option('/timestepping/nonlinear_iterations',1)
  ...
  subroutine form_basis_mp_all()
    !!< Matrices containing the snapshots for arpack
    !      type(state_type), dimension(:), allocatable :: state
    real, dimension(:,:,:,:), allocatable :: snapmatrix_velocity, snapmatrix_velocity_deim
    real, dimension(:,:,:), allocatable :: snapmatrix_pressure, snapmatrix_pressure_deim,snapmatrix_volumefraction
    real, dimension(:,:,:,:), allocatable :: leftsvd_velocity, leftsvd_velocity_deim
    real, dimension(:,:,:), allocatable :: leftsvd_pressure, leftsvd_pressure_deim,leftsvd_volumefraction
    real, dimension(:,:,:), allocatable :: svdval_velocity, svdval_velocity_deim
    real, dimension(:,:), allocatable :: svdval_pressure, svdval_pressure_deim,svdval_volumefraction
    real, dimension(:,:,:), allocatable :: snapmean_velocity, snapmean_velocity_deim
    real, dimension(:,:), allocatable :: snapmean_pressure, snapmean_pressure_deim,snapmean_volumefraction
    integer :: snapshots, u_nodes, p_nodes, nsvd,v_nodes
    integer :: i,dump_no, d, dim,j,k ,m,i2,i3
    integer :: stat    
    real, dimension(:,:), allocatable :: P !n*m (u_nodes*nsvd)
    !integer, dimension(:), allocatable :: phi
    ...
    call get_option(&
         '/reduced_model/pod_basis_formation/pod_basis_count', nsvd)
    call get_option('/simulation_name',simulation_name)
    simulation_name1='all'
    numphase=option_count("/material_phase")
    call read_input_states_fromallvtu(state,state_p) 

    ! do  p=1,numphase
       call retrieve_snapshots_mp(state, snapshots, u_nodes, p_nodes, v_nodes, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction, &
                                & snapmean_velocity, snapmean_pressure, snapmean_volumefraction)    
       call form_svd_mp(snapmatrix_velocity, snapmatrix_pressure,snapmatrix_volumefraction,&
         & leftsvd_velocity, leftsvd_pressure, leftsvd_volumefraction, svdval_velocity, svdval_pressure, svdval_volumefraction, snapshots)
        
       call form_podstate_velocity(state,pod_state,leftsvd_velocity,leftsvd_pressure, snapmean_velocity, snapmean_pressure)
       call form_podstate_pressure(state_p,pod_state_p,leftsvd_velocity,leftsvd_pressure, snapmean_velocity, snapmean_pressure)
       call form_podstate_volumefraction(state_p,pod_state_v,leftsvd_volumefraction, snapmean_volumefraction)
    ! enddo
    call add_option("/reduced_model/execute_reduced_model",stat)
    ...
    do i=1,nsvd 
       ...
       !  call vtk_write_pod_state_velocity(filename=trim(simulation_name)//"_PODBasisV", index=dump_no, state=pod_state(i,:))
       !  call vtk_write_pod_state_pressure(filename=trim(simulation_name)//"_PODBasisP", index=dump_no, state=pod_state(i,:))
      call vtk_write_state(filename=trim(simulation_name1)//"_"//trim(phase_name)//"_PODBasisVelocity", index=dump_no, model="VelocityMesh", state=pod_state(i,:,m))
      call vtk_write_state(filename=trim(simulation_name1)//"_"//trim(phase_name)//"_PODBasisPressure", index=dump_no, model="PressureMesh", state=pod_state_p(i,:,m))
      call vtk_write_state(filename=trim(simulation_name1)//"_"//trim(phase_name)//"_PODBasisVolumefraction", index=dump_no, model="VolumefractionMesh", state=pod_state_v(i,:,m))
       ! call checkpoint_simulation(pod_state(i,:),prefix="gyre",cp_no=dump_no, postfix="podbasis")
       ! call checkpoint_fields(pod_state(i,:),prefix="gyre",postfix="podbasis",cp_no=dump_no)
       ! call checkpoint_state(pod_state(i,:),prefix="gyre",postfix="podbasis",cp_no=dump_no)       
    enddo
    ...
    ! call write_options(trim(simulation_name)//".mpml")
     ...
     call add_option("/reduced_model/execute_reduced_model",stat)
    call set_option('/simulation_name', trim(simulation_name)//'_POD') 
    call write_options(trim(simulation_name)//"_POD.mpml")
     deallocate(snapmatrix_velocity)
     deallocate(snapmatrix_pressure)
     deallocate(snapmean_velocity)
     deallocate(snapmean_pressure)  
     deallocate(pod_state)
     deallocate(pod_state_p)  
  end subroutine form_basis_mp_all 

    subroutine form_basis_mp()
    !!< Matrices containing the snapshots for arpack
    !      type(state_type), dimension(:), allocatable :: state
    real, dimension(:,:,:,:), allocatable :: snapmatrix_velocity, snapmatrix_velocity_deim
    real, dimension(:,:,:), allocatable :: snapmatrix_pressure, snapmatrix_pressure_deim,snapmatrix_volumefraction
    ...
    do i=1,nsvd 
      ...
      call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVelocity", index=dump_no, model="VelocityMesh", state=pod_state(i,:,m))
      call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisPressure", index=dump_no, model="PressureMesh", state=pod_state_p(i,:,m))
      call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVolumefraction", index=dump_no, model="VolumefractionMesh", state=pod_state_v(i,:,m))
       ! call checkpoint_simulation(pod_state(i,:),prefix="gyre",cp_no=dump_no, postfix="podbasis")
       ! call checkpoint_fields(pod_state(i,:),prefix="gyre",postfix="podbasis",cp_no=dump_no)
       ! call checkpoint_state(pod_state(i,:),prefix="gyre",postfix="podbasis",cp_no=dump_no)       
    enddo
 ...
 subroutine retrieve_snapshots_mp(state, snapshots, u_nodes, p_nodes, v_nodes, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction, &
                                & snapmean_velocity, snapmean_pressure, snapmean_volumefraction)
   !!< 
    type(state_type), intent(in), dimension(:,:) :: state
    real, dimension(:,:,:,:), allocatable :: snapmatrix_velocity
    ...
    ! integer :: numphase
    integer :: snapshots, u_nodes, p_nodes, v_nodes,dim, i, d, p
    type(vector_field), pointer :: velocity
    type(scalar_field), pointer :: pressure, volumefraction  
    
    velocity => extract_vector_field(state(1,1), "Velocity")
    pressure => extract_scalar_field(state_p(1,1), "Pressure")
    !call print_state(state_p)
    !stop 2233
    volumefraction=> extract_scalar_field(state_p(1,1), "PhaseVolumeFraction")
    dim=velocity%dim
    p_nodes=node_count(pressure)
    u_nodes=node_count(velocity)
    ...
    allocate(snapmean_velocity(u_nodes,dim,numphase))
    allocate(snapmean_pressure(p_nodes,numphase))   
    allocate(snapmatrix_volumefraction(v_nodes,snapshots,numphase))
   ...
   do p = 1, numphase
    do i = 1, snapshots
      ...
      ! pressure => extract_scalar_field(state_p(i,p), "Pressure")
      ! pressure => extract_scalar_field(state_p(i,1), "Pressure")
      ! volumefraction=> extract_scalar_field(state_p(i,1), "PhaseVolumeFraction")
        pressure => extract_scalar_field(state_p(i,1), "Pressure")
       volumefraction=> extract_scalar_field(state_p(i,p), "PhaseVolumeFraction")
       do d = 1, dim 
          ! print *,'size(snapmatrix_velocity,1), size(velocity%val,2)',size(snapmatrix_velocity,1), size(velocity%val,2) ,size(velocity%val,1)   
          ! print *,velocity%val(d,:)     
           snapmatrix_velocity(:,i,d,p)=velocity%val(d,:)
           ...
           snapmatrix_pressure(:,i,p)=pressure%val
           snapmatrix_volumefraction(:,i,p)=volumefraction%val!(p,:)
    end do
    ...
    do i=1, snapshots
       ...
       enddo
       snapmean_pressure(:,p)=snapmean_pressure(:,p)+snapmatrix_pressure(:,i,p)
       snapmean_volumefraction(:,p)=snapmean_volumefraction(:,p)+snapmatrix_volumefraction(:,i,p)
    end do
    ...
    do i=1,snapshots
       ...
       enddo
       snapmatrix_pressure(:,i,p)=snapmatrix_pressure(:,i,p)-snapmean_pressure(:,p)
       snapmatrix_volumefraction(:,i,p)=snapmatrix_volumefraction(:,i,p)-snapmean_volumefraction(:,p)
    enddo
    enddo !end numphase  
    !snapmean_volumefraction(:,2)=1-snapmean_volumefraction(:,1)
   !print *, 'snapshotsvalue',snapmatrix_volumefraction 
  end subroutine retrieve_snapshots_mp
...
!    vtu--->state
    ...
    do i=1, total_dumps
        ...
        pressuremesh =extract_mesh(state_p(i,j), "Mesh") 
        call insert(state(i,j), pressuremesh, name="PressureMesh") 
    end do
    enddo  

       ! call print_state(state(1))
        call print_state(state_p(1,1))
       ! print *, 'state_p%mesh_names(:)',state_p%mesh_names(:)
       ! call print_state(state(2))
       ! call print_state(state_p(2))
  end subroutine read_input_states_fromvtu

  subroutine read_input_states_fromallvtu(state,state_p)
    !!< Read the input states from the vtu dumps of the forward run.
    type(state_type), intent(out), dimension(:,:), allocatable :: state
    type(state_type), intent(out), dimension(:,:), allocatable :: state_p
    character(len=1024) :: filename
     type(scalar_field) ::pressure
    integer :: dump_sampling_period, quadrature_degree
    integer :: i,j,k,total_dumps, total_dump, stable_dumps
    type(mesh_type) ::pressuremesh
    integer :: numphase
    character(len=1024) :: phase_name
    CHARACTER(255) dir
    Character(512)::CurWorkDir
    INTEGER(4) length
    call get_option('/reduced_model/pod_basis_formation/dump_sampling_period',dump_sampling_period)
    call get_option('/geometry/quadrature/degree', quadrature_degree)
    ...
    !substract gyre_0.vtu
    total_dump=count_dumps(dump_sampling_period)
    total_dumps=total_dump*41 !number of smolyak nodes
    numphase=option_count("/material_phase")
    allocate(state(total_dumps,numphase))
    allocate(state_p(total_dumps,numphase))
  ...
  ! END IF
   ! open(51,file=trim(dir)//'//temp.txt',form='unformatted', status='unknown')
  !  close(51) 

   ! Call GetCWD(CurWorkDir)
    !Write(*.*) CurWorkDir
    !print *, 'current::', CurWorkDir
  !  stop 235
  ...
  !    vtu--->state
    ...
    do j=1, numphase
         ...
         ! call get_option("/material_phase["//int2str(m)//"]/name", mat_name)
         print *,'phase_name=',  int2str(j), phase_name
         print *, 'total_snapshots',total_dump, total_dumps
      do i=1, total_dumps         
          if (i<=total_dump) then           
           simulation_name='1'
           else if (i>=total_dump .and. i<=2*total_dump) then         
           simulation_name='2'
           else if (i>=2*total_dump .and. i<=3*total_dump) then
           simulation_name='3'
           else if (i>=3*total_dump .and. i<=4*total_dump) then
           simulation_name='4'
           else if (i>=4*total_dump .and. i<=5*total_dump) then
           simulation_name='5'  
         ! if(.false.) then       
           else if (i>=5*total_dump .and. i<=6*total_dump) then
           simulation_name='6' 
           else if (i>=6*total_dump .and. i<=7*total_dump) then
           simulation_name='7' 
           else if (i>=7*total_dump .and. i<=8*total_dump) then
           simulation_name='8' 
           else if (i>=8*total_dump .and. i<=9*total_dump) then
           simulation_name='9' 
            else if (i>=9*total_dump .and. i<=10*total_dump) then
           simulation_name='10' 
           else if (i>=10*total_dump .and. i<=11*total_dump) then
           simulation_name='11' 
           else if (i>=11*total_dump .and. i<=12*total_dump) then
           simulation_name='12' 
           else if (i>=12*total_dump .and. i<=13*total_dump) then
           simulation_name='13'
           else if (i>=13*total_dump .and. i<=14*total_dump) then
           simulation_name='14' 
           else if (i>=14*total_dump .and. i<=15*total_dump) then
           simulation_name='15' 
           else if (i>=15*total_dump .and. i<=16*total_dump) then
           simulation_name='16' 
           else if (i>=16*total_dump .and. i<=17*total_dump) then
           simulation_name='17' 
           else if (i>=17*total_dump .and. i<=18*total_dump) then         
           simulation_name='18'
           else if (i>=18*total_dump .and. i<=19*total_dump) then
           simulation_name='19'
           else if (i>=19*total_dump .and. i<=20*total_dump) then
           simulation_name='20'
           else if (i>=20*total_dump .and. i<=21*total_dump) then
           simulation_name='21' 
           else if (i>=21*total_dump .and. i<=22*total_dump) then
           simulation_name='22' 
           else if (i>=22*total_dump .and. i<=23*total_dump) then
           simulation_name='23' 
           else if (i>=23*total_dump .and. i<=24*total_dump) then
           simulation_name='24' 
           else if (i>=24*total_dump .and. i<=25*total_dump) then
           simulation_name='25' 
           else if (i>=25*total_dump .and. i<=26*total_dump) then
           simulation_name='26' 
           else if (i>=26*total_dump .and. i<=27*total_dump) then
           simulation_name='27' 
           else if (i>=27*total_dump .and. i<=28*total_dump) then
           simulation_name='28' 
           else if (i>=28*total_dump .and. i<=29*total_dump) then
           simulation_name='29'
           else if (i>=29*total_dump .and. i<=30*total_dump) then
           simulation_name='30' 
           else if (i>=30*total_dump .and. i<=31*total_dump) then
           simulation_name='31' 
           else if (i>=31*total_dump .and. i<=32*total_dump) then
           simulation_name='32' 
           else if (i>=32*total_dump .and. i<=33*total_dump) then
           simulation_name='33'
           else if (i>=33*total_dump .and. i<=34*total_dump) then
           simulation_name='34' 
           else if (i>=34*total_dump .and. i<=35*total_dump) then
           simulation_name='35' 
           else if (i>=35*total_dump .and. i<=36*total_dump) then
           simulation_name='36' 
           else if (i>=36*total_dump .and. i<=37*total_dump) then
           simulation_name='37'
           else if (i>=37*total_dump .and. i<=38*total_dump) then
           simulation_name='38' 
           else if (i>=38*total_dump .and. i<=39*total_dump) then
           simulation_name='39' 
           else if (i>=39*total_dump .and. i<=40*total_dump) then
           simulation_name='40' 
           else if (i>=40*total_dump .and. i<=41*total_dump) then
           simulation_name='41'
          endif
          k=mod(i,total_dump)+1
          print *, 'kkkkk,simulation_name',k, simulation_name
          ! stop 33333    
          write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//trim(phase_name)//'_VelocityMesh_', (k)*dump_sampling_period,'_checkpoint',".vtu" 
          call vtk_read_state(filename, state(i,j), quadrature_degree)
          write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//trim(phase_name)//'_PressureMesh_', (k)*dump_sampling_period,'_checkpoint',".vtu" 
          call vtk_read_state(filename, state_p(i,j), quadrature_degree)
    ...
    enddo   
       ! call print_state(state_p(1,1)) 
  end subroutine read_input_states_fromallvtu
