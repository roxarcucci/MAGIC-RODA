...
program non_intrusive_vtu_pvtu_optisime
  use spud
  use fields
  use state_module
...
contains 
  ...
  subroutine nonintrusive_snapshots_main()
   ...
   ! call retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction) 
    call put_pvtu_to_states_to_array(state,state_p,snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction) 
 ...
 !!!  for case has the same mesh like P1_P1
  ...
  subroutine put_pvtu_to_states(state,state_p)
   ...
   ! integer :: numphase
    ...
    total_dumps=15!count_dumps_pvtu(dump_sampling_period)
   ! numphase=option_count("/material_phase")
    allocate(state(no_smolyak_nodes,total_dumps,numphase))
  ...
  subroutine retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
   ...
   subroutine put_pvtu_to_states_to_array(state,state_p,snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction) 
    type(state_type), intent(out), dimension(:,:,:), allocatable :: state
    type(state_type), intent(out), dimension(:,:,:), allocatable :: state_p
    type(state_type), dimension(:), allocatable :: state_temp
    character(len=1024) :: filename
  ...
  allocate(state_temp(1)) 
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".pvtu" 
      print *,'filename=', filename 
       call vtk_read_state(filename, state_temp(1), quadrature_degree) 
       velocity => extract_vector_field(state_temp(1), "Velocity") 
       print *, 'u_nodes=node_count(velocity---iii)', u_nodes, size(velocity%val,2)
       do d = 1, dimen 
           snapmatrix_velocity(k,:,i,d,p)=velocity%val(d,:)
       end do 
        ...
        call vtk_read_state(filename, state_temp(1), quadrature_degree) 
         print *,'filename=', filename 
         pressuremesh =extract_mesh(state_temp(1), "Mesh") 
         call insert(state_temp(1), pressuremesh, name="PressureMesh")  
         pressure_p=extract_scalar_field(state_temp,"Pressure")
         call insert(state_temp(1), pressure_p, name="Pressure") 
         volumefraction_p=extract_scalar_field(state_temp(1),"Pressure")
         !call insert(state(k,1,p),  volumefraction_p, name="Pressure")

         pressure => extract_scalar_field(state_temp(1), "Pressure")
         print *, 'p_nodes=node_count(pressure)', p_nodes
         volumefraction=> extract_scalar_field(state_temp(1), "Pressure")! change
         snapmatrix_pressure(k,:,i,p)=pressure%val
         snapmatrix_volumefraction(k,:,i,p)=volumefraction%val!(p,:)  
...
end program non_intrusive_vtu_pvtu_optisime
