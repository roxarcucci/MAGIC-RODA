 !This includes only velocity field for LSBU case, the original one is a file named Non_intrusive_pvtu20180102.F90 on backup in this machine.
...
contains 
  ...
  subroutine nonintrusive_snapshots_main()
   ...
   allocate(snapmean_pressure(p_nodes,numphase))  
    ...
    call cpu_time(cpu_basis_start)!snapmatrix_velocity, &       & leftsvd_velocity, svdval_velocity,  snapmatrix_pressure, leftsvd_pressure, vdval_pressure, snapshots
    call form_svd_mp(interpolate_velocity, leftsvd_velocity, svdval_velocity, interpolate_pressure, leftsvd_pressure, svdval_pressure,interpolate_volumefraction, leftsvd_volumefraction,svdval_volumefraction,snapshots)
   ...
   allocate (xd(podnum,nd)) 
    ...
    ! visulisation
     ...
     do k=1, numphase 
       ...
       pressure_field => extract_scalar_field(state(1,s,1), "TracerGeorge" )
      volumefraction_field=> extract_scalar_field(state(1,s,1), "TracerWaterloo")!SolidConcentration" )  ! change
      ...
      call vtk_write_state(filename=trim(simulation_name)//'_tracer', index=dump_no, model="Mesh", state=state(:,s,1))
...
!
 ...
 !!!  for case has the same mesh like P1_P1
  ...
  subroutine put_pvtu_to_states(state,state_p)
   ...
   ! numphase=option_count("/material_phase")
    allocate(state(no_smolyak_nodes,total_dumps+300,numphase))
    allocate(state_p(no_smolyak_nodes,total_dumps+300,numphase))
    ...
    ! initialise the predicitve parts of the state, in order to allocate for the predictive values
     do i=total_dumps+1, total_dumps+10
...
!
  ...
  return
   ...
   !!< 
    ...
    velocity => extract_vector_field(state(1,1,1), "Velocity")
     pressure => extract_scalar_field(state(1,1,1), "TracerGeorge") !(state_p(1,1,1), "Pressure") 
     volumefraction=> extract_scalar_field(state(1,1,1), "TracerWaterloo")!(state_p(1,1,1), "SolidConcentration")! change
    dimen=velocity%dim
     p_nodes=node_count(pressure)
    u_nodes=node_count(velocity)
     v_nodes=node_count(volumefraction)
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       print *, 'u_nodes=node_count(velocity---iii)', u_nodes, size(velocity%val,2)
        pressure => extract_scalar_field(state(k,i,1), "TracerGeorge")
      ! print *, 'p_nodes=node_count(pressure)', p_nodes
        volumefraction=> extract_scalar_field(state(k,i,p), "TracerWaterloo")! change
       do d = 1, dimen 
           snapmatrix_velocity(k,:,i,d,p)=velocity%val(d,:)
       end do 
          snapmatrix_pressure(k,:,i,p)=pressure%val
  ...
  subroutine form_svd_mp(snapmatrix_velocity, &
       & leftsvd_velocity, svdval_velocity,  snapmatrix_pressure, leftsvd_pressure, svdval_pressure, snapmatrix_volumefraction, leftsvd_volumefraction,svdval_volumefraction,snapshots)
   ...
   ! type(state_type), pointer, dimension(:):: state
    ...
    do i = 1,POD_num
      ...
      ! pod_mesh => extract_mesh(state(1), "Mesh")
       ...
       call insert(pod_state_p(i,1,p), pod_pmesh, " Mesh")
