...
contains
  ...
  total_timestep=int((finish_time-current_time)/dt)
    ...
    call read_basis_states(state_full, state_pod) 
      do kk =1, POD_num!total_dumps!,ntime,int_dump_period
         ...
         !  vf_full=>extract_scalar_field (state_full(kk), "PODVelocity (PODVelocity)") ! 1 donates phase 1.
         !  vf_pod=>extract_scalar_field (state_pod(kk), "SnapmeanVelocity (SnapmeanVelocity)") 
         !  u_full=>extract_vector_field (state_full(kk), "PODVelocity") ! 1 donates phase 1.
         !  u_pod=>extract_vector_field  (state_pod(kk), "PODVelocity") 

              vf_full=>extract_scalar_field (state_full(kk), "PODPressure") ! 1 donates phase 1.
              vf_pod=>extract_scalar_field  (state_pod(kk),  "PODPressure") 
  ...
  open(unit=60,file='rmse_jcp')
   ...
   subroutine calculate_initial_norm2() 
    integer :: snapshots, u_nodes, p_nodes, nsvd
    integer :: i,dump_no, d, dim, kk
    integer :: stat 
    type(vector_field), pointer :: u_full,u_pod
    ...
    !  do kk =1, POD_num!total_dumps!,ntime,int_dump_period
         ...
         !  vf_pod=>extract_scalar_field (state_pod(kk), "SnapmeanVelocity (SnapmeanVelocity)") 
            ...
            !  u_pod=>extract_vector_field (state_pod(kk), "PODVelocity") 
              ...
              vf_full=>extract_scalar_field (state_full(kk), "PODVolumefraction") ! 1 donates phase 1.
              vf_pod=>extract_scalar_field (state_pod(kk), "PODVolumefraction") 
              u_nodes=node_count(vf_full)  
                            nodesum=0
                            fullnodesum=0
                            podnodesum=0
                        ...
                        do i=1,u_nodes
                            ...
                            podnodesum=podnodesum+nodeval_pod(1)!coor
                        enddo
                            rmse(kk)=SQRT(nodesum/u_nodes)   ! write the equations here
                            nrmse(kk)=rmse(kk)/(y_max-y_min)
                           ! rmseone=SQRT(nodesum/u_nodes) 
                      coor_xy=0 
                      coor_x=0
  ...
  open(unit=60,file='rmse_jcp')
   ...
   enddo 
   close(62)
  end subroutine calculate_initial_norm2


  subroutine calculate_initial_error() 
    integer :: snapshots, u_nodes, p_nodes, nsvd
    integer :: i,dump_no, d, dim, kk
    integer :: stat 
    type(vector_field), pointer :: u_full,u_pod
  ...
  open(unit=60,file='rmse_jcp')
   ...
   enddo 
   close(62)
  end subroutine calculate_initial_error
 ...
 subroutine read_basis_states(state_full, state_pod)
    ...
    do i=1, POD_num      
       ...
       ! write(filename, '(a, i0, a)') trim(simulation_name)//"_"//trim(phase_name)//'_PODBasisPressure_', i,".vtu" !Volumefraction 
       write(filename, '(a, i0, a)') trim(simulation_name)//"_"//trim(phase_name)//'_PODBasisVolumefraction_', i,".vtu" !Volumefraction   
       call vtk_read_state(filename, state_full(i), quadrature_degree) 
       ! write(filename, '(a, i0, a)') trim(simulation_name)//"_"//trim(phase_name)//'_PODBasisVelocity_meanend_', i,".vtu"  
       ! write(filename, '(a, i0, a)') trim(simulation_name)//"_"//trim(phase_name)//'_PODBasisPressure_meanend_', i,".vtu" 
       write(filename, '(a, i0, a)') trim(simulation_name)//"_"//trim(phase_name)//'_PODBasisVolumefraction_meanend_', i,".vtu"   
       call vtk_read_state(filename, state_pod(i), quadrature_degree) 
     end do
   ! fracture3_phase1_PODBasisVelocity_meanend_23.vtu
  end subroutine read_basis_states
