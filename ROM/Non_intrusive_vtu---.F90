    ...
    ! visulisation
     ...
     do k=1, numphase 
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
        ...
        pressure_field => extract_scalar_field(state(1,s,1), "ComponentMassFractionPhase1" )
        volumefraction_field=> extract_scalar_field(state(1,s,1), "ComponentMassFractionPhase1")!SolidConcentration" )  ! change
 ...
 !!!  for case has the same mesh like P1_P1
   ...
   ! numphase=option_count("/material_phase")
    ...
    do k=1, no_smolyak_nodes
     ...
     do i=1, total_dumps
        ...
        pressure=extract_scalar_field(state_p(k,i,j),"ComponentMassFractionPhase1")
       call insert(state(k,i,j),  pressure, name="ComponentMassFractionPhase1")
    
       volumefraction=extract_scalar_field(state_p(k,i,j),"ComponentMassFractionPhase1")
        call insert(state(k,i,j),  volumefraction, name="ComponentMassFractionPhase1")
  ...
  !  numphase=option_count("/material_phase")
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    ...
    pressure => extract_scalar_field(state(1,1,1), "ComponentMassFractionPhase1") !(state_p(1,1,1), "Pressure") 
    volumefraction=> extract_scalar_field(state(1,1,1), "ComponentMassFractionPhase1")!(state_p(1,1,1), "SolidConcentration")! change
    dimen=velocity%dim
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       pressure => extract_scalar_field(state(k,i,1), "ComponentMassFractionPhase1")
       print *, 'p_nodes=node_count(pressure)', p_nodes
       volumefraction=> extract_scalar_field(state(k,i,p), "ComponentMassFractionPhase1")! change
       do d = 1, dimen 
