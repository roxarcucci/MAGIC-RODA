...
!#include "finclude/petsc.h"
  ...
  type(state_type), dimension(:,:,:), allocatable :: pod_state, result_state 
...
contains 
   subroutine interpolate_channel_22d_main()
    ...
    sparse_max=1
    no_smolyak_nodes=22
   allocate(input_parameters(no_smolyak_nodes+1, m))
 
   open(unit=61, file="input_permeability")   
     read(61,*)((input_parameters(j,k),k=1,m), j=1,no_smolyak_nodes+1)  
    close(61) 
    ...
    if(.true.) then
     ...
     do i=1,snapshots 
       dump_no=i 
      call vtk_write_state(filename=trim(simulation_name)//"_interp"//"Velocity", index=dump_no, model="Mesh", state=state(no_smolyak_nodes,i,:))
      call vtk_write_state(filename=trim(simulation_name)//"_interp"//"Pressure", index=dump_no, model="PressureMesh", state=state(no_smolyak_nodes,i,:))
      call vtk_write_state(filename=trim(simulation_name)//"_interp"//"Volumefraction", index=dump_no, model="PressureMesh", state=state(no_smolyak_nodes,i,:))
                     ...
                     enddo   
                       if((s .eq. 1).and.(k.eq.1)) then                       
                    	open(11,file="coef_pod_all_obv"//"_"//trim(simulation_name))  
                           ...
                           print *, 'total_timestep-pressure-vf---timestep===', simulation_name,snapshots, s 
                   	close(11) 
                    else 
                   	open(11,file="coef_pod_all_obv"//"_"//trim(simulation_name), position='append',ACTION='WRITE')  
                         write(11,*)(pod_coef_all_obv(s,:,k))
  ...
  subroutine non_intrusive_22d()
    ...
    real , allocatable :: fe(:,:) !exact values of function produced from fluidity
    real ( kind = 8 ), allocatable :: fi(:) 
    real, dimension(:,:,:), allocatable :: pressure_val,volumefraction_val
    real :: maxall,minall
    integer :: m,ll,kk,u_nonods,l,nd, podnum,ni
    numphase=option_count("/material_phase")
    total_timestep=total_dumps
   ...
   allocate (xd(podnum,nd)) 
     ...
     do l=1, m
      ...
      call rbf_weight ( m, nd, xd, r0, phi4, fd, wm(:,l) )
     enddo
     pressure_field => extract_scalar_field(state(1,1,1), "Pressure" ) 
     volumefraction_field=> extract_scalar_field(state(1,1,1), "PhaseVolumeFraction" )
     allocate(pressure_val(size(pressure_field%val),total_timestep, numphase))
     allocate(volumefraction_val(size(pressure_field%val),total_timestep, numphase)) 
    pod_coef_old=pod_coef_all(1,:)
    DO s=1, total_timestep!=total_dumps     
         xi(:,1)= pod_coef_old !pod_coef_all_obv(kk,:)pod_coef_all(s,:) ! 
         ! print *,'xiiiiiiiiiii=',  xi
     ...
     enddo  !do l=1, m      
      ! visulisation
      ! print *, 'pod_coef_new', pod_coef_new
      ! state(no_smolyak_nodes,total_dumps,numphase)
     do k=1, numphase 
      ! pressure_field => extract_scalar_field(state_show(k), "Pressure" )
       pressure_field => extract_scalar_field(state(no_smolyak_nodes,s,k), "Pressure" )
       !volumefraction_field=> extract_scalar_field(state_show(k), "PhaseVolumeFraction" )  
       volumefraction_field=> extract_scalar_field(state(no_smolyak_nodes,s,k), "PhaseVolumeFraction" )
       pressure_field%val(:)=0
       volumefraction_field%val(:)=0  
           do i=1,nsvd  
               ! print *, 'difference', pod_coef_new(i+(k-1)*(m/numphase))-pod_coef_all_obv(s,i,k)
               ...
               ! volumefraction_field%val(:)=volumefraction_field%val(:)+pod_coef_new(i+nsvd+(k-1)*(m/numphase))*leftsvd_volumefraction(:,i,k)  
           enddo 
      pressure_val(:,s,k)=pressure_field%val(:)
      volumefraction_val(:,s,k)= volumefraction_field%val(:)
      snapmean_velocity=0
      snapmean_pressure=0
      snapmean_volumefraction=0 
      pressure_field%val=pressure_field%val(:)+ snapmean_pressure(:,k)   
     call insert(state(no_smolyak_nodes,s,k), pressure_field, "pressure")
     volumefraction_field%val=volumefraction_field%val+snapmean_volumefraction(:,k)
     call insert(state(no_smolyak_nodes,s,k), volumefraction_field, "PhaseVolumeFraction")
    ...
    ENDDO ! Do s=1, total_timestep
     ...
     call form_result_state_22d(state(no_smolyak_nodes,:,:), state_p(no_smolyak_nodes,:,:), result_state, pressure_val, volumefraction_val)
     print *, 'simulation_name', simulation_name
    ! call print_state(result_state(1,1,1))
     !call print_state(result_state(1,2,1))
    ! call print_state(result_state(1,3,1))
     do ph=1, 1
      do i=1,total_timestep
       dump_no=i 
       call vtk_write_state(filename=trim(simulation_name)//"_final", index=dump_no, model="Mesh", state=result_state(i,:,ph))!pod_state(i,1,ph)
       !call vtk_write_state(filename=trim(simulation_name)//"_final", index=dump_no, model="PressureMesh", state=result_state(i,2,:))
      ! call vtk_write_state(filename=trim(simulation_name)//"_final", index=dump_no, model="VolumefractionMesh", state=result_state(i,3,:)) 
     enddo
     enddo !numphase   
     deallocate(pod_coef_new) 
     deallocate(pod_coef_old) 
     ...
     deallocate(xd)
     deallocate(w)
     deallocate(pressure_val)
     deallocate(volumefraction_val)
...
subroutine read_input_states(state,state_p)
 ...
 subroutine  form_result_state_22d(state, state_p, result_state, pressure_val, volumefraction_val)
   
    type(state_type), intent(in), dimension(:,:) :: state, state_p   
    type(state_type), intent(out), dimension(:,:,:), allocatable :: result_state
    real, intent(in), dimension(:,:,:) :: pressure_val, volumefraction_val
   
   ! type(scalar_field),intent(in):: pressure_field, volumefraction_field
    type(mesh_type), pointer :: pod_xmesh, pod_umesh, pod_pmesh, pmesh, pod_mesh 
    type(vector_field), pointer :: pod_positions, velocity
   
    type(vector_field) :: result_velocity
    type(scalar_field) :: result_pressure, result_volumefraction 
    character(len = FIELD_NAME_LEN) :: field_name 
    integer :: dump_sampling_period, quadrature_degree,nonods
    integer :: i,j,k,nod, stat,f,d,p
    logical :: all_meshes_same
    integer :: pod_pnodes,pod_unodes 
    allocate(result_state(total_dumps,3,numphase))
    call nullify(pod_state)
    ...
    do i = 1, total_dumps
      ...
      ! pod_mesh => extract_mesh(state(1), "Mesh")
        ...
        pod_umesh =>extract_mesh(state(1,p), "Mesh")
        pod_positions => extract_vector_field(state(1,p), "Coordinate") 
       call insert(result_state(i,1,p), pod_umesh, "CoordinateMesh")
       call insert(result_state(i,1,p), pod_umesh, "Mesh")    
       call insert(result_state(i,1,p), pod_positions, "Coordinate") 
       velocity => extract_vector_field(state(1,p), "phase"//int2str(p)//"::Velocity")
       call allocate(result_velocity, velocity%dim, pod_umesh, "resultVelocity")
       call zero(result_velocity)
       do d=1,velocity%dim
         ! call set_all(result_velocity, d, leftsvd_u(:,i,d,p))
       end do
       call insert(result_state(i,1,p), result_velocity, name="resultVelocity")
       call deallocate(result_velocity) 
       

       pod_xmesh => extract_mesh(state_p(1,p), "Mesh")
       pod_pmesh =>extract_mesh(state_p(1,p), "Mesh")
       pod_positions => extract_vector_field(state_p(1,p), "Coordinate")
       call insert(result_state(i,2,p), pod_xmesh, "CoordinateMesh") 
       call insert(result_state(i,2,p), pod_pmesh, "Mesh")
       call insert(result_state(i,2,p), pod_positions, "Coordinate")  
        call allocate(result_pressure, pod_pmesh, "resultPressure")    
       call zero(result_pressure) 
       result_pressure%val(:)=pressure_val(:,i,p)
       call insert(result_state(i,2,p), result_pressure, name="resultPressure") 
       call deallocate(result_pressure)

       pod_xmesh => extract_mesh(state_p(1,p), "Mesh") 
       pod_pmesh =>extract_mesh(state_p(1,p), "Mesh")  
       pod_positions => extract_vector_field(state_p(1,p), "Coordinate") 
       call insert(result_state(i,3,p), pod_xmesh, "CoordinateMesh") 
       call insert(result_state(i,3,p), pod_pmesh, "Mesh")
       call insert(result_state(i,3,p), pod_positions, "Coordinate") 
       call allocate(result_volumefraction, pod_pmesh, "resultVolumefraction")    
       call zero(result_volumefraction)
       result_volumefraction%val(:)=volumefraction_val(:,i,p)
       ! call set_all(result_volumefraction, leftsvd_v(:,i,p))
       call insert(result_state(i,3,p), result_volumefraction, name="resultVolumefraction")
     
       call deallocate(result_volumefraction)
   
    enddo  
     enddo !numphase 
  
  end subroutine form_result_state_22d
  ...
  Do u_nodesi=1, u_nodes 
   ...
   do numphasei=1, 1!size(snapmatrix_velocity,5)       
     ...
     ! call rbf_weight ( m, nd, xd, r0, phi4, fd, wm(:,1) )
       ...
       xi(:,1)=input_parameters(no_smolyak_nodes+1,:)
...
!
  ...
  !smolynode, u_nodes,snapshots,dim,numphase
  Do u_nodesi=1, p_nodes  
      print * ,'volumefraction nodes' , u_nodesi, p_nodes 
   Do snapshotsi=1, snapshots  
   do numphasei=1, 1! numphase 
   do no_smolyak_nodesi=1, no_smolyak_nodes
            do mi=1, m
