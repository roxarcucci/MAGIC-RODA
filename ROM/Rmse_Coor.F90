...
#include "finclude/petsc.h"
  ...
  !  call calculate_rsme_coor_velocity()
  !  call calculate_rsme_coor_pressure()
  !  call calculate_rsme_coor_density()
  !  call calculate_rsme_coor_solidc()
...
contains
  subroutine calculate_rsme_coor_mp() 
   ...
   ! call put_checkpoint_vtu_to_states(state_full, state_pod)
    ...
    do kk =1, total_dumps!,ntime,int_dump_period
        u_full => extract_vector_field(state_full(kk), "Velocity")
        u_pod => extract_vector_field(state_pod(kk), "Velocity")   
              u_nodes=node_count(u_full)  
                        ...
                        do i=1,u_nodes
                           ...
                           ! print *,'nodeval_full(2)nodeval_pod', nodeval_full(2),nodeval_pod(2)
                           ! stop 11                                      
  ...
  do i=1, total_dumps
   ...
   enddo
   close(61) 
  open(unit=60,file='rmse')
   do i=1, total_dumps
    ! write(60,*) ((i-1)*4+100)*2, rmse(i)
     write(60,*) i, rmse(i)
   enddo 
   close(60)
   open(unit=62,file='nrmse')
   do i=1, total_dumps
    ! write(60,*) ((i-1)*4+100)*2, rmse(i)
     write(62,*) i, nrmse(i)
   enddo 
 ...
 subroutine calculate_rsme_coor_velocity() 
    integer :: snapshots, u_nodes, p_nodes, nsvd
    integer :: i,dump_no, d, dim, kk
    integer :: stat 
    type(vector_field), pointer :: u_full,u_pod
    type(scalar_field), pointer :: vf_full,vf_pod
    real ::  nodesum, rmse_initial, sum_full_nodes, rmse_initial_divide_sum_node !RMSE
    integer  :: ntime
    integer dump_no_tmp, int_dump_period,total_timestep
    real current_time,finish_time,dt
    real :: nodeval_full(2),nodeval_pod(2),rmsetemp,coor(15000),fullnodesum,podnodesum,coor_xy,coor_x,coor_y , rmse(15000), nrmse(15000),y_max,y_min, y_sum
     ...
     ! do kk =1, total_dumps!,ntime,int_dump_period
      ...
      !  vf_pod=>extract_scalar_field (state_pod(kk), "phase1::PhaseVolumeFraction") 
         ...
         !  enddo !do kk =1, ntime,int_dump_period
              ...
              u_full=>extract_vector_field (state_full(total_dumps), "Velocity") ! 1 donates phase 1.
              u_pod=>extract_vector_field (state_pod(total_dumps-1), "Velocity") 
              u_nodes=node_count(u_full)  
                            nodesum=0
                            fullnodesum=0 
                            y_max=0
                        ...
                        do i=1,u_nodes
                            ...
                            fullnodesum= fullnodesum+nodeval_full(1) !coor 
                        enddo
  rmse_initial_divide_sum_node=  rmse_initial/fullnodesum               
  open(unit=61,file='coorelation')
  do i=1, total_dumps 
     write(61,*) i, coor(i)! depends on what you want to draw the numer
   enddo
  ...
  open(unit=60,file='rmse')
   ...
   open(unit=63,file='rmse_velocity_initial')
   write(63,*) rmse_initial, rmse_initial_divide_sum_node! depends on what you want to draw the number 
   close(63)
  end subroutine calculate_rsme_coor_velocity

  subroutine calculate_rsme_coor_solidc() 
    integer :: snapshots, u_nodes, p_nodes, nsvd
    integer :: i,dump_no, d, dim, kk
    integer :: stat 
    type(vector_field), pointer :: u_full,u_pod
    type(scalar_field), pointer :: vf_full,vf_pod
    real ::  nodesum, rmse_initial, rmse_initial_divide_sum_node !RMSE
    integer  :: ntime
    integer dump_no_tmp, int_dump_period,total_timestep
    real current_time,finish_time,dt
    real :: nodeval_full(2),nodeval_pod(2),rmsetemp,coor(15000),fullnodesum,podnodesum,coor_xy,coor_x,coor_y , rmse(15000), nrmse(15000),y_max,y_min, y_sum
  ...
  open(unit=60,file='rmse')
   ...
   open(unit=63,file='rmse_solidc_initial')
   write(63,*) rmse_initial, rmse_initial_divide_sum_node  ! depends on what you want to draw the number 
   close(63)

end subroutine calculate_rsme_coor_solidc


 subroutine calculate_rsme_coor_pressure() 
    integer :: snapshots, u_nodes, p_nodes, nsvd
    integer :: i,dump_no, d, dim, kk
    integer :: stat 
    type(vector_field), pointer :: u_full,u_pod
    ...
    !  do kk =1, total_dumps!,ntime,int_dump_period
        kk=1
         vf_full=>extract_scalar_field (state_full(kk+1), "Pressure") ! 1 donates phase 1.
        vf_pod=>extract_scalar_field (state_pod(kk), "Pressure") 
            !  u_full=>extract_vector_field (state_full(kk), "Velocity") ! 1 donates phase 1.
             ! u_pod=>extract_vector_field (state_pod(kk), "Velocity") 
              u_nodes=node_count(vf_full)  
                            nodesum=0
                            fullnodesum=0
   ...
   ! enddo !do kk =1, ntime,int_dump_period
           ...
           vf_full=>extract_scalar_field (state_full(total_dumps), "Pressure") ! 1 donates phase 1.
           vf_pod=>extract_scalar_field (state_pod(total_dumps-1), "Pressure")  
              u_nodes=node_count(vf_full)  
                            nodesum=0
                            fullnodesum=0
 ...
 rmse_initial_divide_sum_node=rmse_initial/fullnodesum
  ...
  open(unit=60,file='rmse')
   ...
   open(unit=63,file='rmse_pressure_initial')
   write(63,*) rmse_initial, rmse_initial_divide_sum_node  ! depends on what you want to draw the number 
   close(63)
  end subroutine calculate_rsme_coor_pressure

 subroutine calculate_rsme_coor_density() 
    integer :: snapshots, u_nodes, p_nodes, nsvd
    integer :: i,dump_no, d, dim, kk
    integer :: stat 
    type(vector_field), pointer :: u_full,u_pod
  ...
  open(unit=60,file='rmse')
   ...
   open(unit=63,file='rmse_density_initial')
   write(63,*) rmse_initial, rmse_initial_divide_sum_node  ! depends on what you want to draw the number 
   close(63)
  end subroutine calculate_rsme_coor_density
...
subroutine read_input_states(state_full, state_pod) 
    ...
    total_dumps=total_dumps/2 ! for checkpoint twice the size of original
    ...
    do i=1, total_dumps  
     ...
     !  write(filename, '(a, i0, a)') trim(simulation_name)//'_', (i)*dump_sampling_period,".vtu"  
       ...
       write(filename, '(a, i0, a)') trim(simulation_name)//'_POD_', i*2,".vtu"  !coupling
  ...
  subroutine put_checkpoint_vtu_to_states(state_full, state_pod)
    !!< Read the input states from the vtu dumps of the forward run. ! different mesh such as P1_dg_P2
    type(state_type), intent(out), dimension(:), allocatable :: state_full, state_pod
    type(state_type), dimension(:), allocatable :: state_scalar
    character(len=1024) :: filename
    ...
    allocate(state_full(total_dumps)) 
    allocate(state_pod(total_dumps)) 
    allocate(state_scalar(total_dumps))
    !substract gyre_0.vtu
    total_dumps=count_dumps(dump_sampling_period)
  !  numphase=option_count("/material_phase")
    
    
     do i=1, total_dumps  !fracture3_VelocityMesh_994_checkpoint.vtu
       write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'VelocityMesh_',  i,'_checkpoint',".vtu"  
       call vtk_read_state(filename, state_full(i), quadrature_degree) 
        write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_'//'PressureMesh_', i,'_checkpoint',".vtu" 
        call vtk_read_state(filename, state_scalar(i), quadrature_degree) 
       write(filename, '(a, i0, a)') trim(simulation_name)//'_POD_', i,".vtu"  !coupling 
       call vtk_read_state(filename, state_pod(i), quadrature_degree) 

       pressuremesh =extract_mesh(state_scalar(i), "Mesh") 
        call insert(state_full(i), pressuremesh, name="PressureMesh") 
      
        pressure=extract_scalar_field(state_scalar(i),"Pressure")
       call insert(state_full(i),  pressure, name="Pressure")
    
       volumefraction=extract_scalar_field(state_scalar(i),"Density")
        call insert(state_full(i),  volumefraction, name="Density")
     end do 
    
    call deallocate(state_scalar)
   end subroutine put_checkpoint_vtu_to_states
