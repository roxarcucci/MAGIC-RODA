...
contains 
  ...
  subroutine nonintrusive_snapshots_main()
   ...
   ! call put_pvtu_to_states(state,state_p)  
    call retrieve_nodesvalue_from_states(state, snapmatrix_velocity ) 
  ...
  !  allocate(interpolate_pressure(p_nodes,snapshots,numphase)) 
   ! allocate(interpolate_volumefraction(p_nodes,snapshots,numphase))
    ...
    call form_svd_mp(interpolate_velocity, leftsvd_velocity, svdval_velocity, snapshots)
    ...
    integer :: i, j, k,s 
       ...
       ! snapmean_pressure=0
        ! snapmean_volumefraction=0 
           ...
           allocate(pod_coef_all_obv(snapshots,dimen*nsvd,numphase))  
            do s=1, snapshots
                 do k=1, numphase 
...
!
  ...
  return
   ...
   subroutine retrieve_nodesvalue_from_states(state, snapmatrix_velocity)
  ...
  subroutine form_svd_mp(snapmatrix_velocity, &
       & leftsvd_velocity,  svdval_velocity,  snapshots)
