...
contains 
   ...
   allocate (xd(podnum,nd)) 
    ...
    ! visulisation
     ...
     do k=1, numphase 
       ...
       volumefraction_field=> extract_scalar_field(state(1,1,1), "Temperature" )  
     ...
     call insert(state_show(k), pressure_field, "PODPressure")
     volumefraction_field%val=volumefraction_field%val+snapmean_volumefraction(:,k)
     call insert(state_show(k), volumefraction_field, "PODTemperature")
      do d=1,dimen
  ...
  subroutine put_vtu_to_states(state,state_p)
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    ...
    volumefraction=> extract_scalar_field(state(1,1,1), "Temperature")!(state_p(1,1,1), "PhaseVolumeFraction")
    dimen=velocity%dim
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       volumefraction=> extract_scalar_field(state(k,i,p), "Temperature")
       do d = 1, dimen 
