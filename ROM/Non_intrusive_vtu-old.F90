...
#include "finclude/petsc.h"
  ...
  integer :: nsvd
    real :: start_formbasis, finish_formbasis, start_weight, finish_weight
#ifdef HAVE_MPI
  call mpi_init(ierr)
#endif
...
contains 
  ...
  subroutine nonintrusive_snapshots_main()
    ...
    call cpu_time(finish_formbasis) 
    print *, 'time of forming_svd', finish_formbasis-start_formbasis 
  ...
  subroutine non_intrusive()
   ...
   allocate (xd(podnum,nd)) 
     ...
     enddo
     call cpu_time(finish_weight)
     print *, 'time of obtaining weighting', finish_weight-start_weight
     stop 2121 
     if(.false.) then
     ! setting the initial begin
       ...
       volumefraction_initial=> extract_scalar_field(state_initial, "PhaseVolumeFraction")!c
       do i=1,size(POD_state,1)
         do j=1,dimen !velocity%dim 
    ...
    ! visulisation
     ...
     do k=1, numphase 
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
       ...
       volumefraction_field=> extract_scalar_field(state(1,s,k), "PhaseVolumeFraction" )  !c
       ...
       !nomean
        ...
        snapmean_volumefraction=0
          ! do i=1,nsvd  
                ...
                ! pressure_field%val(:)=pressure_field%val(:)+pod_coef_new(i+(k-1)*(m/numphase))*leftsvd_pressure(:,i,k)!i+dimen*nsvd
                ! volumefraction_field%val(:)=volumefraction_field%val(:)+pod_coef_new(i+nsvd+(k-1)*(m/numphase))*leftsvd_volumefraction(:,i,k)!i+(dimen+1)*nsvd
     ...
     call insert(state_show(k), volumefraction_field, "PhaseVolumeFraction")!c
      do d=1,dimen
  ...
  subroutine put_vtu_to_states(state,state_p)
   ...
   !!< 
    ...
    volumefraction=> extract_scalar_field(state(1,1,1), "PhaseVolumeFraction")!(state_p(1,1,1), "PhaseVolumeFraction")!c
    dimen=velocity%dim
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       volumefraction=> extract_scalar_field(state(k,i,p), "PhaseVolumeFraction")!c
       do d = 1, dimen 
