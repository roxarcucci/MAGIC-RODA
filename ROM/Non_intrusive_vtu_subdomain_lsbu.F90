...
program subdomain_non_intrusive_vtu_lsbu
  use spud
  use fields
  use state_module
  ...
  type(state_type), dimension(:), allocatable :: state_id
  ...
  real, dimension(:,:,:,:,:), allocatable :: snapmatrix_pressure_subdomain, snapmatrix_volumefraction_subdomain
  ...
  real, dimension(:,:,:,:), allocatable :: pod_coef_all_obv_subdomain
  integer :: nsvd, temp_id
  real :: finish_time
  integer ::  total_timestep, unodes_sub, pnodes_sub
...
!#endif
  ...
  nodes_positions1=>extract_vector_field(state_show, "Coordinate")
  dimen=nodes_positions1%dim
  
 ! if (dimen .eq. 3) then
 !  call nonintrusive_subdomains_main_3d()
 !  else 
 !  call nonintrusive_subdomains_main()
 ! endif
...
contains
    subroutine nonintrusive_subdomains_main()
    !!< Matrices containing the snapshots for arpack                 !old
    real, dimension(:,:,:,:,:,:), allocatable :: snapmatrix_velocity_subdomain
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field
    integer :: i, dump_no, d, j, k ,m,i2,i3,ph,p,  isubdomain,s
    integer :: stat
    type(vector_field), pointer :: nodes_positions
    !type vector_field, dimension(:) :: nodes_positions
    ...
    integer, dimension(:,:), allocatable :: subdomain_number_nodeid
    ...
    numphase=1
    number_subdomainx=15
    allocate(nodes_sum_each(number_subdomainx))

    call initialise_write_state
   ! call populate_state(state_show)
    call read_input_states(state)
    nodes_positions=>extract_vector_field(state_show, "Coordinate")
    dimen=nodes_positions%dim
    call retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
    
    allocate(subdomain_number_nodeid(number_subdomainx, size(nodes_positions%val,2)))
    allocate(nodes_mesh(size(nodes_positions%val,2), snapshots))

    !  print *, 'nodes', size(nodes_positions%val,1), size(nodes_positions%val,2), nodes_positions%val(1,1)
    
    do p=1, size(nodes_positions%val,1)
    ! print *, 'xy', p, nodes_positions%val(p,1), nodes_positions%val(p,2), nodes_positions%val(p,3), nodes_positions%val(p,3213),    nodes_positions%val(p,3213)
    ! print *, 'xy'
    enddo
    ...
    ! print *, 'size(nodes_positions%val,2)', size(nodes_positions%val,2)
    ...
    nodes_positions=>extract_vector_field(state(:,1,1), "Coordinate")
    print *, 'nodes',  nodes_positions%val(1,1)
    
    ! test two subdomains
    allocate(snapmatrix_velocity_subdomain(number_subdomainx,no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    allocate(snapmatrix_pressure_subdomain(number_subdomainx,no_smolyak_nodes,p_nodes,snapshots,numphase))
    allocate(snapmatrix_volumefraction_subdomain(number_subdomainx,no_smolyak_nodes,v_nodes,snapshots,numphase))


   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!retrieve_nodesvalue_from_staes!!!!!!!!!!!!!!!!!!!
   ! print *, 'size(nodes_positions%val,2)', size(nodes_positions%val,2)
    ...
    nodes_sum_each(:)=0
    do i = 1, snapshots
       velocity => extract_vector_field(state(1,i,1), "Velocity")
       pressure => extract_scalar_field(state(1,i,1), "Pressure")
       volumefraction=> extract_scalar_field(state(1,i,1), "Pressure")!c
    do j=1, size(nodes_positions%val,2)! nodes number
      nodes_mesh(j,i)%id=j
      nodes_mesh(j,i)%pressure=pressure%val(j)
      nodes_mesh(j,i)%volumefraction=pressure%val(j)
        do k=1, dimen
            nodes_mesh(j,i)%velocity(k)=velocity%val(k,j)
        enddo
     if (nodes_positions%val(1,j) .LE. (maxx/2)) then!1.01
        nodes_mesh(j,i)%domain_id=1
        if (i.eq.1) then
          nodes_sum_each(1)=nodes_sum_each(1)+1
          nodes_mesh(j,i)%sequence=nodes_sum_each(1) 
        endif
         nodes_mesh(j,i)%sequence=nodes_mesh(j,1)%sequence
         subdomain_number_nodeid(1,j)= j
     else
         nodes_mesh(j,i)%domain_id=2
         !subdomain_number_nodeid(2,j)= j         
         if ( i.eq.1) then
            nodes_sum_each(2)=nodes_sum_each(2)+1
            nodes_mesh(j,i)%sequence=nodes_sum_each(2)
         !  print *, 'nodes_sum_each(2)', nodes_sum_each(2)
         !  print *, 'nodes_mesh(j,i)%sequence', nodes_mesh(j,i)%sequence !nodes_mesh(j,i)%velocity(1), nodes_mesh(j,i)%domain_id, nodes_mesh(j,i)%id
         !  print *,'nodes_positions%x', nodes_positions%val(1,j), nodes_positions%val(2,j)
          endif
          nodes_mesh(j,i)%sequence=nodes_mesh(j,1)%sequence
     endif
    enddo
    enddo
   !  print *,'nodes', nodes_sum_each(2)+nodes_sum_each(1)-size(nodes_positions%val,2) 
    ...
    do j=1, size(nodes_positions%val,2)! nodes number
      ...
      nodes_mesh(j,i)%volumefraction=pressure%val(j)
        do k=1, dimen
            nodes_mesh(j,i)%velocity(k)=velocity%val(k,j)
        enddo
     if (nodes_positions%val(1,j) .LE. (maxx/15) .and. nodes_positions%val(2,j) .GE. (maxy*3/4) ) then!1.01
        nodes_mesh(j,i)%domain_id=1 
      else if (nodes_positions%val(1,j) .GT. (maxx/15) .and. nodes_positions%val(1,j) .LE. (maxx*3/15) .and. nodes_positions%val(2,j) .GE. (maxy*3/4) ) then
        nodes_mesh(j,i)%domain_id=2  
      else if (nodes_positions%val(1,j) .GT. (maxx*3/15) .and. nodes_positions%val(1,j) .LE. (maxx/2) .and. nodes_positions%val(2,j) .GE. (maxy*3/4) ) then
        nodes_mesh(j,i)%domain_id=3  
      else if (nodes_positions%val(1,j) .GT. (maxx/2) .and. nodes_positions%val(1,j) .LE. (maxx*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*3/4) ) then
        nodes_mesh(j,i)%domain_id=4
       else if (nodes_positions%val(1,j) .GT. (maxx*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*3/4) ) then
        nodes_mesh(j,i)%domain_id=5 

       else if (nodes_positions%val(1,j) .LE. (maxx/15) .and. nodes_positions%val(2,j) .LT. (maxy*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*1/4)) then
        nodes_mesh(j,i)%domain_id=6  
      else if (nodes_positions%val(1,j) .GT. (maxx/15) .and. nodes_positions%val(1,j) .LE. (maxx*3/15) .and. nodes_positions%val(2,j) .LT. (maxy*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*1/4) ) then
        nodes_mesh(j,i)%domain_id=7  
      else if (nodes_positions%val(1,j) .GT. (maxx*3/15) .and. nodes_positions%val(1,j) .LE. (maxx*1/2) .and. nodes_positions%val(2,j) .LT. (maxy*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*1/4) ) then
        nodes_mesh(j,i)%domain_id=8
      else if (nodes_positions%val(1,j) .GT. (maxx/2) .and. nodes_positions%val(1,j) .LE. (maxx*3/4) .and. nodes_positions%val(2,j) .LT. (maxy*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*1/4) ) then
        nodes_mesh(j,i)%domain_id=9
       else if (nodes_positions%val(1,j) .GT. (maxx*3/4) .and. nodes_positions%val(2,j) .LT. (maxy*3/4) .and. nodes_positions%val(2,j) .GE. (maxy*1/4) ) then
        nodes_mesh(j,i)%domain_id=10  

      else if (nodes_positions%val(1,j) .LE. (maxx/15) .and. nodes_positions%val(2,j) .LT. (maxy/4) ) then
        nodes_mesh(j,i)%domain_id=11  
      else if (nodes_positions%val(1,j) .GT. (maxx/15) .and. nodes_positions%val(1,j) .LE. (maxx*3/15) .and. nodes_positions%val(2,j) .LT. (maxy/4) ) then
        nodes_mesh(j,i)%domain_id=12
       else if (nodes_positions%val(1,j) .GT. (maxx*3/15) .and. nodes_positions%val(1,j) .LE. (maxx*1/2) .and. nodes_positions%val(2,j) .LT. (maxy/4) ) then
        nodes_mesh(j,i)%domain_id=13
      else if (nodes_positions%val(1,j) .GT. (maxx/2) .and. nodes_positions%val(1,j) .LE. (maxx*3/4) .and. nodes_positions%val(2,j) .LT. (maxy/4) ) then
        nodes_mesh(j,i)%domain_id=14  
      else if (nodes_positions%val(1,j) .GT. (maxx*3/4) .and. nodes_positions%val(2,j) .LT. (maxy/4) ) then
        nodes_mesh(j,i)%domain_id=15  
          
     endif
    enddo
    enddo

   ! specify  nodes_mesh_subdomain
   do ii=1, number_subdomainx  
    allocate(nodes_mesh_subdomain(u_nodes, snapshots))
   ! allocate(interpolate_velocity(u_nodes,snapshots,dimen,numphase)) !need more
    !allocate(interpolate_pressure(u_nodes,snapshots,numphase))
   ! allocate(interpolate_volumefraction(u_nodes,snapshots,numphase))
    nodes_mesh_subdomain(:,:)%domain_id=ii
    do k=1, 3
      nodes_mesh_subdomain(:,:)%velocity(k)=0
    enddo
    nodes_mesh_subdomain(:,:)%pressure=0
    nodes_mesh_subdomain(:,:)%volumefraction=0
     do s=1, snapshots
     do j=1, size(nodes_positions%val,2) ! nodes number
         if(nodes_mesh(j,s)%domain_id .eq.ii) then 
            do k=1, dimen
             ! interpolate_velocity(nodes_mesh(j,s)%sequence,s,k,1)=nodes_mesh(j,s)%velocity(k)
             ! nodes_mesh_subdomain(nodes_mesh(j,s)%sequence,s)%velocity(k)=nodes_mesh(j,s)%velocity(k)
             nodes_mesh_subdomain(j,s)%velocity(k)=nodes_mesh(j,s)%velocity(k)
            enddo
            ! interpolate_pressure(nodes_mesh(j,s)%sequence,s,1)=nodes_mesh(j,s)%pressure
            !  interpolate_volumefraction(nodes_mesh(j,s)%sequence,s,1)=nodes_mesh(j,s)%volumefraction
              nodes_mesh_subdomain(nodes_mesh(j,s)%sequence,s)%id=nodes_mesh(j,s)%id
             
             !  print *, 'nodes_mesh(j,i)%', nodes_mesh(j,s)%domain_id, nodes_mesh(j,s)%id
             !nodes_mesh_subdomain(nodes_mesh(j,s)%sequence,s)%pressure=nodes_mesh(j,s)%pressure
             !nodes_mesh_subdomain(nodes_mesh(j,s)%sequence,s)%volumefraction=nodes_mesh(j,s)%volumefraction
              nodes_mesh_subdomain(j,s)%pressure=nodes_mesh(j,s)%pressure
              nodes_mesh_subdomain(j,s)%volumefraction=nodes_mesh(j,s)%volumefraction
             !print *, 'nodes_mesh(j,s)%sequence, ', nodes_mesh(j,s)%sequence
        endif
     enddo
    enddo
   
    
    call form_svd_subdomain(ii,u_nodes, p_nodes, nodes_mesh_subdomain,&
    ...
    allocate(snapmean_volumefraction(p_nodes,numphase))
    snapmean_velocity=0
    snapmean_pressure=0
    ...
    do i=1,nsvd
        dump_no=i
       call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVelocity"//"subdomain"//trim(int2str(ii)), index=dump_no, model="VelocityMesh", state=pod_state(i,:,ph))
       call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisPressure"//"subdomain"//trim(int2str(ii)), index=dump_no, model="PressureMesh", state=pod_state_p(i,:,ph))
       call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVolumefraction"//"subdomain"//trim(int2str(ii)), index=dump_no, model="VolumefractionMesh", state=pod_state_v(i,:,ph))

    enddo
    enddo !numphase
    ! call calculate_pod_coef_all_timesteps()
     call calculate_pod_coef_all_timesteps_subdomain(ii)
     ! call non_intrusive_subdomain(ii,unodes_sub, nodes_mesh_subdomain) !!!
     call non_intrusive_subdomains(ii,nodes_mesh_subdomain)
     ! call non_intrusive()
   
     deallocate(nodes_mesh_subdomain)
   ...
   enddo
    ...
    deallocate(snapmatrix_velocity_subdomain)
    deallocate(snapmatrix_pressure_subdomain)
    deallocate(snapmatrix_volumefraction_subdomain)  
    deallocate(nodes_sum_each)
    deallocate(subdomain_number_nodeid)
    deallocate(nodes_mesh)
    deallocate(state)
  end subroutine nonintrusive_subdomains_main
  
 subroutine nonintrusive_subdomains_main_lsbu()
    !!< Matrices containing the snapshots for arpack                 !old
    real, dimension(:,:,:,:,:,:), allocatable :: snapmatrix_velocity_subdomain
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field
    integer :: i, dump_no, d, j, k, m, i2, i3, ph, p, isubdomain,s,jj
    integer :: stat
    type(vector_field), pointer :: nodes_positions, nodes_positions_sub
    !type vector_field, dimension(:) :: nodes_positions
    integer :: udim,deim_number_tmp
    type(vector_field), pointer :: velocityudim
    type(vector_field), pointer :: vfield
    ...
    total_subdomain = 32 
    call initialise_write_state
    ! call populate_state(state_id)
    call read_subdomain_id(state_id) 
    call read_input_states_lsbu(state)
    nodes_positions=>extract_vector_field(state_show, "Coordinate")
    dimen=nodes_positions%dim
    allocate(nodes_sum_each(total_subdomain))
    call retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
     !  print *, 'nodes', size(nodes_positions%val,1), size(nodes_positions%val,2), nodes_positions%val(1,1)
     ! do p=1, size(nodes_positions%val,1)
    ! print *, 'xy', p, nodes_positions%val(p,1), nodes_positions%val(p,2), nodes_positions%val(p,3), nodes_positions%val(p,3213),    nodes_positions%val(p,3213)
    ...
    nodes_positions=>extract_vector_field(state(:,1,1), "Coordinate")
    print *, 'nodes',  nodes_positions%val(1,1), size(nodes_positions%val,2) 
    allocate(snapmatrix_velocity_subdomain(total_subdomain,no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    allocate(snapmatrix_pressure_subdomain(total_subdomain,no_smolyak_nodes,p_nodes,snapshots,numphase))
    allocate(snapmatrix_volumefraction_subdomain(total_subdomain,no_smolyak_nodes,v_nodes,snapshots,numphase)) 

   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!retrieve_nodesvalue_from_sta !!!!!!!!!!!!!!!!!!!
   ! print *, 'size(nodes_positions%val,2)', size(nodes_positions%val,2)
    velocity => extract_vector_field(state(1,1,1), "Velocity")
    pressure => extract_scalar_field(state(1,1,1), "Pressure")
    p_nodes=node_count(pressure)
    ...
    allocate(nodes_mesh(u_nodes, snapshots))  
    ! assigning subdomain-id

 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!obtaining subdomainid   
   do  j=1, size(nodes_positions%val,2)! nodes number
      do ii=1, total_subdomain
       call retrieve_subdomain_id_from_state_id(state_id(ii), ii, snapmatrix_id)
       nodes_positions_sub=>extract_vector_field(state_id(ii), "Coordinate")
       do jj=1, size(snapmatrix_id) 
        if ((nodes_positions%val(1,j) .eq. nodes_positions_sub%val(1,jj)) .and.(nodes_positions%val(2,j) .eq. nodes_positions_sub%val(2,jj)) .and. (nodes_positions%val(3,j) .eq. nodes_positions_sub%val(3,jj))) then!1.01
            nodes_mesh(j,1)%domain_id=ii
            !exit !exit jj
          goto 30 
    !30 continue
        endif 
       enddo 
    30   deallocate(snapmatrix_id)
       !exit !exit ii
       enddo
       enddo
       do i = 1, snapshots
         nodes_mesh(:,i)%domain_id=nodes_mesh(:,1)%domain_id
       enddo
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!obtaining subdomainid  

    do ii=1, total_subdomain
	call retrieve_subdomain_id_from_state_id(state_id(ii), ii, snapmatrix_id)
        !ids => extract_scalar_field(state_id(ii), "id")
        !print *, 'ids', ii, ids%val
         nodes_sum_each(ii)=size(snapmatrix_id)
     ...
     do i = 1, snapshots
      ...
      ! do jj=1, size(snapmatrix_id) 
           ...
           !if (j .eq. snapmatrix_id(jj)) then
           !  nodes_mesh(j,i)%domain_id=ii 
          ! endif 

         !dividing the domain into 2 parts, testing
        ! if (nodes_positions%val(1,j) .LE. (maxx/2)) then!1.01
        !     nodes_mesh(j,i)%domain_id=1 
        !  else 
        !   nodes_mesh(j,i)%domain_id=2
        !  endif
           ...
           !print *, 'nodes_mesh(j,i)%domain_id', size(snapmatrix_id), j, i, nodes_mesh(j,i)%domain_id
      ! enddo !do jj=1, size(snapmatrix_id)
      nodes_mesh(j,i)%id=j
      nodes_mesh(j,i)%pressure=pressure%val(j)
      nodes_mesh(j,i)%volumefraction=pressure%val(j)
        do k=1, dimen
            nodes_mesh(j,i)%velocity(k)=velocity%val(k,j)
        enddo
      ! print *, 'subdomain-id', nodes_mesh(j,i)%domain_id
     enddo ! do j=1, size(nodes_positions%val,2)!
    enddo ! do i = 1, snapshots
    deallocate(snapmatrix_id)
   enddo !do ii=1, total_subdomain
   ...
   enddo
    ...
    deallocate(state)
    deallocate(nodes_sum_each)
  end subroutine nonintrusive_subdomains_main_lsbu
  

   subroutine nonintrusive_subdomains_main_3d()
    !!< Matrices containing the snapshots for arpack                 !old
    real, dimension(:,:,:,:,:,:), allocatable :: snapmatrix_velocity_subdomain
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field
    integer :: i, dump_no, d, j, k ,m,i2,i3,ph,p, total_subdomain, isubdomain,s
    integer :: stat
    type(vector_field), pointer :: nodes_positions
    !type vector_field, dimension(:) :: nodes_positions
    integer :: udim,deim_number_tmp
    ...
    nodes_positions=>extract_vector_field(state_show, "Coordinate")
     dimen=nodes_positions%dim
    call retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
    print *,    size(nodes_positions%val,2)
    ! stop 212
    allocate(subdomain_number_nodeid(number_subdomainx, size(nodes_positions%val,2)))
    allocate(nodes_mesh(size(nodes_positions%val,2), snapshots))
    ...
    !  print *, 'nodes', size(nodes_positions%val,1), size(nodes_positions%val,2), nodes_positions%val(1,1)
     ...
     minx=nodes_positions%val(1,1)
     miny=nodes_positions%val(2,1)
     minz=nodes_positions%val(3,1)
     maxx=nodes_positions%val(1,1)
     maxy=nodes_positions%val(2,1)
     maxz=nodes_positions%val(3,1)
    ! print *, 'size(nodes_positions%val,2)', size(nodes_positions%val,2)
     Do i=1, size(nodes_positions%val,2) !number of nodes
           if(minx>nodes_positions%val(1,i)) then
              minx=nodes_positions%val(1,i)
     ...
     do i=1, size(nodes_positions%val,2)
      ...
      enddo
     print *, 'minx, maxx, miny, maxy, minz,maxz', minx, maxx, miny, maxy, minz,maxz
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!retrieve_nodesvalue_from_states!!!!!!!!!!!!!!!!!!!
   ! print *, 'size(nodes_positions%val,2)', size(nodes_positions%val,2)
    velocity => extract_vector_field(state(1,1,1), "Velocity")
    pressure => extract_scalar_field(state(1,1,1), "Pressure")
    p_nodes=node_count(pressure)
   ...
   do i = 1,  snapshots
       ...
       pressure => extract_scalar_field(state(1,i,1), "Pressure")
       volumefraction=> extract_scalar_field(state(1,i,1), "Pressure")!c
       !print *, 'size(nodes_positions%val,2)',  size(nodes_positions%val,2) !9905
    do j=1, size(nodes_positions%val,2)! nodes number
      nodes_mesh(j,i)%id=j
      nodes_mesh(j,i)%pressure=pressure%val(j)
      nodes_mesh(j,i)%volumefraction=pressure%val(j)
        do k=1, dimen
            nodes_mesh(j,i)%velocity(k)=velocity%val(k,j)
        enddo
     !print *, 'nodes_positions%val(:,j)', nodes_positions%val(1,j), nodes_positions%val(2,j), nodes_positions%val(3,j)

       nodes_mesh(j,i)%domain_id=1 
     if (nodes_positions%val(1,j) .LE. (maxx/4) .and. nodes_positions%val(3,j) .LE. (maxz/2) ) then!1.01
         nodes_mesh(j,i)%domain_id=1 
      else if (nodes_positions%val(1,j) .GT. (maxx/4) .and. nodes_positions%val(1,j) .LE. (maxx/2) .and. nodes_positions%val(3,j) .LE. (maxz/2) ) then
         nodes_mesh(j,i)%domain_id=2  
      else if (nodes_positions%val(1,j) .GT. (maxx/2) .and. nodes_positions%val(1,j) .LE. (maxx*3/4) .and. nodes_positions%val(3,j) .LE. (maxz/2) ) then!1.01
         nodes_mesh(j,i)%domain_id=3 
      else if (nodes_positions%val(1,j) .GT. (maxx*3/4)   .and. nodes_positions%val(3,j) .LE. (maxz/2) ) then
         nodes_mesh(j,i)%domain_id=4  
      else if (nodes_positions%val(1,j) .LE. (maxx/2)  .and. nodes_positions%val(3,j) .GE. (maxz/2) ) then
        nodes_mesh(j,i)%domain_id=5  
      else if (nodes_positions%val(1,j) .GT. (maxx/2)   .and. nodes_positions%val(3,j) .GE. (maxz/2) ) then
         nodes_mesh(j,i)%domain_id=6
   ...
   ! specify  nodes_mesh_subdomain
   do ii=1, number_subdomainx 
     print *, 'iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii', ii
    allocate(nodes_mesh_subdomain(u_nodes, snapshots))
   ! allocate(interpolate_velocity(u_nodes,snapshots,dimen,numphase)) !need more
    !allocate(interpolate_pressure(u_nodes,snapshots,numphase))
   ! allocate(interpolate_volumefraction(u_nodes,snapshots,numphase))
    ...
    nodes_mesh_subdomain(:,:)%volumefraction=0
     ...
     do j=1, size(nodes_positions%val,2) ! nodes number
         if(nodes_mesh(j,s)%domain_id .eq.ii) then 
            do k=1, dimen            
             nodes_mesh_subdomain(j,s)%velocity(k)=nodes_mesh(j,s)%velocity(k)
             !print *, 'nodes_mesh_subdomain', nodes_mesh_subdomain(j,s)%velocity(k)
            enddo 
              nodes_mesh_subdomain(nodes_mesh(j,s)%sequence,s)%id=nodes_mesh(j,s)%id 
              nodes_mesh_subdomain(j,s)%pressure=nodes_mesh(j,s)%pressure
              nodes_mesh_subdomain(j,s)%volumefraction=nodes_mesh(j,s)%volumefraction 
   ...
   enddo
    ...
    deallocate(nodes_mesh)
    deallocate(state)
  end subroutine nonintrusive_subdomains_main_3d

  subroutine nonintrusive_subdomain_main()
    !!< Matrices containing the snapshots for arpack                 !old
    real, dimension(:,:,:,:,:,:), allocatable :: snapmatrix_velocity_subdomain
    type(vector_field), pointer :: velocity_field
    type(scalar_field), pointer :: pressure_field, volumefraction_field
    ...
    ! stop 1121
    ! flow past a cylinder, 2.2**0.409
    ! call print_state(state_show(1))
    nodes_positions=>extract_vector_field(state(:,1,1), "Coordinate")
    print *, 'nodes',  nodes_positions%val(1,1)
    ! vector%val(1,1) picker_inquire_single
    ! call picker_inquire(node_positions,
   ...
   do j=1, size(nodes_positions%val,2)
      ...
      nodes_mesh(j,i)%volumefraction=pressure%val(j)
        do k=1, dimen
            nodes_mesh(j,i)%velocity(k)=velocity%val(k,j)
        enddo
     if (nodes_positions%val(1,j) .LE. (maxx/1.05)) then!1.01
        nodes_mesh(j,i)%domain_id=1
        if (i.eq.1) then
          nodes_sum_each(1)=nodes_sum_each(1)+1
          nodes_mesh(j,i)%sequence=nodes_sum_each(1)
          ! print *, 'nodes_sum_each(1)', nodes_sum_each(1)
          ! print *,'nodes_positions%val(1,j)',j, nodes_positions%val(1,j)
           !print *, 'nodes_mesh(j,i)%1', nodes_mesh(j,i)%sequence
        endif
         nodes_mesh(j,i)%sequence=nodes_mesh(j,1)%sequence
         subdomain_number_nodeid(1,j)= j
     else
  ...
  ! nodes_sum_each(2)=size(nodes_positions%val,2)-nodes_sum_each(1)


   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

   ! allocate(snapmatrix_velocity(unodes_sub,snapshots,dimen,numphase)) !here it is the interpolate_velocity in subdomain
   ! allocate(snapmatrix_pressure(pnodes_sub,snapshots,numphase))
   ! allocate(snapmatrix_volumefraction(pnodes_sub,snapshots,numphase))
   ...
   do ii=1, number_subdomainx
    unodes_sub=nodes_sum_each(ii)
    pnodes_sub=nodes_sum_each(ii) 
    allocate(nodes_mesh_subdomain(unodes_sub, snapshots))
    allocate(interpolate_velocity(unodes_sub,snapshots,dimen,numphase)) !need more
    allocate(interpolate_pressure(pnodes_sub,snapshots,numphase))
    allocate(interpolate_volumefraction(pnodes_sub,snapshots,numphase))
    nodes_mesh_subdomain(:,:)%domain_id=ii
     do s=1, snapshots
     do j=1, size(nodes_positions%val,2) !nodes number
         if(nodes_mesh(j,s)%domain_id .eq.ii) then
            !nodes_mesh_subdomain(j,s)%sequence=nodes_mesh(j,s)%sequence
            do k=1, dimen
             ! interpolate_velocity(nodes_mesh(j,s)%sequence,s,k,1)=nodes_mesh(j,s)%velocity(k)
              nodes_mesh_subdomain(nodes_mesh(j,s)%sequence,s)%velocity(k)=nodes_mesh(j,s)%velocity(k)
            enddo
   ...
   !  call retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
   !  print *, 'comparison', nodes_mesh_subdomain(200,20)%velocity(2), snapmatrix_velocity(1,200,20,2,1)
   ! do s=1, total_dumps
   !  open(11,file="nodes_mesh_subdomain") 
   !  write(11,*)(nodes_mesh_subdomain(1000:3212,6:26)%velocity(1)) 
   ! close(11)  
   ! open(11,file="snapmatrix_velocity") 
   !  write(11,*)(snapmatrix_velocity(1,1000:3212,6:26,1,1)) 
   ! close(11)
   ! enddo
   ! print *, 'comparison', nodes_mesh_subdomain(2,2)%velocity(2), snapmatrix_velocity(1,:,:,2,1)
   ! stop 5888
   ! allocate(snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
   ! call form_svd_subdomain(ii,unodes_sub, pnodes_sub, interpolate_velocity, interpolate_pressure, interpolate_volumefraction,&
        ! & leftsvd_velocity, leftsvd_pressure, leftsvd_volumefraction, svdval_velocity, svdval_pressure, svdval_volumefraction, snapshots)
    if (ii .eq. 1) then
        do k=1, 3
         ! nodes_mesh_subdomain(:,:)%velocity(k)=0
    ...
    if (ii .eq. 2) then
      ! print *, 'nodes_mesh_subdomainvelocity', nodes_mesh_subdomain(:,1)%velocity(1)
       !print *, 'nodes_mesh_subdomain-id', nodes_mesh_subdomain(:,1)%id
       !   print *, 'nodes_mesh_subdomain-sequence',nodes_mesh(:,1)%sequence
      ! print *, 'nodes_mesh_subdomain-domainid', nodes_mesh_subdomain(:,1)%domain_id
    endif


   ! print *, 'unodes_sub, pnodes_sub', unodes_sub, pnodes_sub
    call form_svd_subdomain(ii,unodes_sub, pnodes_sub, nodes_mesh_subdomain,&
        & leftsvd_velocity, leftsvd_pressure, leftsvd_volumefraction, svdval_velocity, svdval_pressure, svdval_volumefraction, snapshots)
    
    allocate(snapmean_velocity(unodes_sub,dimen,numphase))
    allocate(snapmean_pressure(pnodes_sub,numphase))
    allocate(snapmean_volumefraction(pnodes_sub,numphase))
    snapmean_velocity=0
    snapmean_pressure=0
    snapmean_volumefraction=0
    !call form_podstate_velocity(state(1,:,:),pod_state,leftsvd_velocity,leftsvd_pressure, snapmean_velocity, snapmean_pressure)
   ...
   enddo
    ...
    deallocate(nodes_mesh)
    deallocate(state)
  end subroutine nonintrusive_subdomain_main

 subroutine form_svd_subdomain(isubdomain, unodes_sub, pnodes_sub, nodes_mesh_subdomain,&
       & leftsvd_velocity, leftsvd_pressure, leftsvd_volumefraction, svdval_velocity, svdval_pressure, svdval_volumefraction, snapshots)
    ...
    integer i, d, dim ,nsvd, snapshots, pnodes_sub, unodes_sub, p, isubdomain
    type(single_node), dimension(:,:) :: nodes_mesh_subdomain

    call get_option(&
         '/reduced_model/pod_basis_formation/pod_basis_count', nsvd)
   ! vnodes_sub=pnodes_sub

    !dim=size(snapmatrix_velocity,3)
    allocate(leftsvd_velocity(unodes_sub,nsvd,dimen,numphase))
    allocate(leftsvd_pressure(pnodes_sub,nsvd,numphase))
    allocate(svdval_velocity(nsvd,dimen,numphase))
    allocate(svdval_pressure(nsvd,numphase))
    allocate(leftsvd_volumefraction(pnodes_sub,nsvd,numphase))
    allocate(svdval_volumefraction(nsvd,numphase)) 
    do p=1,numphase
    do d=1,dimen
      ! call snapsvd(unodes_sub,snapshots,snapmatrix_velocity(:,:,d,p),nsvd,nsvd,leftsvd_velocity(:,:,d,p),svdval_velocity(:,d,p))
      call snapsvd(unodes_sub,snapshots,nodes_mesh_subdomain(:,:)%velocity(d),nsvd,nsvd,leftsvd_velocity(:,:,d,p),svdval_velocity(:,d,p))
    end do

    !call snapsvd(pnodes_sub,snapshots,snapmatrix_pressure(:,:,p),nsvd,nsvd,leftsvd_pressure(:,:,p),svdval_pressure(:,p))
    ! call snapsvd(pnodes_sub,snapshots,snapmatrix_volumefraction(:,:,p),nsvd,nsvd,leftsvd_volumefraction(:,:,p),svdval_volumefraction(:,p))
    call snapsvd(pnodes_sub,snapshots,nodes_mesh_subdomain(:,:)%pressure,nsvd,nsvd,leftsvd_pressure(:,:,p),svdval_pressure(:,p))
    call snapsvd(pnodes_sub,snapshots,nodes_mesh_subdomain(:,:)%volumefraction,nsvd,nsvd,leftsvd_volumefraction(:,:,p),svdval_volumefraction(:,p))
    enddo !numphase
   if (.false.) then
    open(11,file="leftsvd_pressure_subdomain"//int2str(isubdomain)) 
     write(11,*)(leftsvd_pressure(:,:,1)) 
    close(11)  
     open(11,file="leftsvd_velocity_subdomain"//int2str(isubdomain)) 
     write(11,*)(leftsvd_velocity(:,:,:,1)) 
    close(11)  
   endif
  end subroutine form_svd_subdomain
 

 subroutine calculate_pod_coef_all_timesteps_subdomain(ii)
      ...
      ! if (have_option("/reduced_model/Velocity_Interpolation")) then
           allocate(pod_coef_all_obv_subdomain(total_subdomain,snapshots,(dimen+2)*nsvd,numphase))
            do s=1, snapshots
                 do k=1, numphase
                   do i=1,nsvd!size(POD_state,1)
                             !romdim=velocity%dim
                             do j=1,dimen!velocity%dim
                             !  pod_coef_all_obv_subdomain(ii,s,i+nsvd*(j-1),k)=&
                                !        dot_product(leftsvd_velocity(:,i,j,k),(interpolate_velocity(:,s,j,k)-snapmean_velocity(:,j,k)))
                                                                     !(:,i,1,ph)(u_nodes,snapshots,dim,numphase))
                              pod_coef_all_obv_subdomain(ii,s,i+nsvd*(j-1),k)=&
                                         dot_product(leftsvd_velocity(:,i,j,k),(nodes_mesh_subdomain(:,s)%velocity(j)))
                             enddo
                            ! pod_coef_all_obv_subdomain(ii,s,i+nsvd*dimen,k)= &
                                  !      dot_product(leftsvd_pressure(:,i,k),(interpolate_pressure(:,s,k)-snapmean_pressure(:,k)))
                            ! pod_coef_all_obv_subdomain(ii,s,i+nsvd*(dimen+1),k)= &
                                   !     dot_product(leftsvd_volumefraction(:,i,k),(interpolate_volumefraction(:,s,k)-snapmean_volumefraction(:,k)))
                              pod_coef_all_obv_subdomain(ii,s,i+nsvd*dimen,k)= &
                                      dot_product(leftsvd_pressure(:,i,k),(nodes_mesh_subdomain(:,s)%pressure)) 
                            pod_coef_all_obv_subdomain(ii,s,i+nsvd*(dimen+1),k)= &
                                        dot_product(leftsvd_volumefraction(:,i,k),(nodes_mesh_subdomain(:,s)%volumefraction))
                     enddo
                       if((s .eq. 1).and.(k.eq.1)) then
                    	open(11,file="coef_pod_all_obv"//"_"//trim(simulation_name)//"subdomain"//trim(int2str(ii)))
                            ! write(11,*)(pod_coef_all_obv(s,1:nsvd*(dimen+1),k)) !no volumefraction
                            write(11,*)(pod_coef_all_obv_subdomain(ii,s,:,k))
                            ! print *, 'total_timestep----timestep===',simulation_name, snapshots, s
                   	close(11)
                    else
                   	open(11,file="coef_pod_all_obv"//"_"//trim(simulation_name)//"subdomain"//trim(int2str(ii)), position='append',ACTION='WRITE')
                         write(11,*)(pod_coef_all_obv_subdomain(ii,s,:,k))
                         !print *, 'total_timestep----timestep===', simulation_name, snapshots, s
                   	close(11)
                     endif
                  ...
                  enddo !numphase
              enddo
    deallocate(pod_coef_all_obv_subdomain) 
  end subroutine calculate_pod_coef_all_timesteps_subdomain

   subroutine non_intrusive_subdomains(ii, nodes_mesh_subdomain)
    type(vector_field), pointer :: velocity_field, velocity_initial
    type(scalar_field), pointer :: pressure_field, volumefraction_field, pressure_initial, volumefraction_initial
    type(state_type):: state_initial
    !integer, intent(in) :: unodes_sub
    !real, dimension(:,:,:,:),intent(in) :: snapmatrix_velocity
    type(single_node), dimension(:,:) :: nodes_mesh_subdomain
    integer, intent(in) :: ii
    real, dimension(:), allocatable ::  pod_coef_orig,pod_coef_new,pod_coef_old
    integer :: i, ph,j,k,s,dump_no, d, iiteration
    real, allocatable :: w(:)
    real, allocatable :: wm(:,:)
    real :: r0
    ...
    real , allocatable :: velocity_subdomain(:,:)
    real , allocatable :: pressure_subdomain(:)
    real , allocatable :: volumefraction_subdomain(:)
   ...
   ! endif
    ...
    podnum =m
     ...
     allocate(w(m))
     allocate(velocity_subdomain(dimen,nodes_sum_each(ii)))
     allocate(pressure_subdomain(nodes_sum_each(ii)))
     allocate(volumefraction_subdomain(nodes_sum_each(ii)))

       do d=1,dimen
           nodes_mesh_subdomain(:,:)%velocity(d)=0
         enddo
         nodes_mesh_subdomain(:,:)%pressure=0
         nodes_mesh_subdomain(:,:)%volumefraction=0

     open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name)//"subdomain"//int2str(ii))
   ...
   allocate (xd(podnum,nd))
     ...
     ! if (ii .eq. 1) then
      !   call set_option('/simulation_name', trim(simulation_name)//'_PODfinal')!//int2str(ii)
      call set_option('/simulation_name', trim(simulation_name)//int2str(ii))!
     ! endif
      call get_option('/simulation_name',simulation_name)
     !  print *,'differnce',  pod_coef_old-pod_coef_all(1,:)
     ...
     do k=1, numphase
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
       if(s <= total_dumps) then
        velocity_field => extract_vector_field(state(1,s,k), "Velocity" )
        pressure_field => extract_scalar_field(state(1,s,k), "Pressure" )
        volumefraction_field=> extract_scalar_field(state(1,s,k), "Pressure" )  !c
        else
        ! velocity_field => extract_vector_field(state(1,s,k), "Velocity" )
        ! pressure_field => extract_scalar_field(state(1,s,k), "Pressure" )
        ! volumefraction_field=> extract_scalar_field(state(1,s,k), "Pressure" )  !c
       ...
       volumefraction_field%val(:)=0  !(dimen+2)*nsvd*numphase=m
       endif
       !nomean
         snapmean_pressure=0
         snapmean_velocity=0
         snapmean_volumefraction=0
          ...
          ! print *, 'size(velocity_field%val,2)',  size(velocity_field%val,2)
           ! print *, 'leftsvd_velocity', size(leftsvd_velocity,1)
           do i=1,nsvd
                do d=1,velocity_field%dim
                   !  print *, 'difference', pod_coef_new(i+(d-1)*nsvd)-pod_coef_all_obv(s,i+(d-1)*nsvd,k)interpolate_velocity(:,s,j,k)(u_nodes,snapshots,dim,numphase))
                !interpolate_velocity(nodes_mesh(j,s)%sequence,s,k,1)=nodes_mesh(j,s)%velocity(k)
                       if( s <= total_dumps) then
                         ! velocity_subdomain(d,:)=interpolate_velocity(:,s,d,k)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                        ! velocity_subdomain(d,:)=nodes_mesh_subdomain(:,s)%velocity(d)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)velocity_field%val(d,:)=velocity_field%val(d,:)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                        nodes_mesh_subdomain(:,s)%velocity(d)=nodes_mesh_subdomain(:,s)%velocity(d)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                ! print *, 'leftsvd_velocity(:,i,d,k)',  size(leftsvd_velocity,1), leftsvd_velocity(:,i,d,k) 
                         ! print *, 'nodes_mesh', size(nodes_mesh,1)
                         ! nodes_mesh(:,s)%velocity(d)=interpolate_velocity(:,s,d,k)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                     !  print *, 'nodes_mesh_subdomain(:,s)%velocity(d)', nodes_mesh_subdomain(:,s)%velocity(d)
                       else
                        nodes_mesh_subdomain(:,s)%velocity(d)=nodes_mesh_subdomain(:,s)%velocity(d)+pod_coef_new(i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k) 
                         ! velocity_subdomain(d,:)=nodes_mesh_subdomain(:,s)%velocity(d)+pod_coef_new(i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                        !  nodes_mesh(:,s)%velocity(d)=interpolate_velocity(:,s,d,k)+pod_coef_new(i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                      endif

                enddo
                     if( s <= total_dumps) then
                     ! pressure_subdomain(:)=nodes_mesh_subdomain(:,s)%pressure+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_pressure(:,i,k)  
                     ! volumefraction_subdomain(:)=nodes_mesh_subdomain(:,s)%volumefraction+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_volumefraction(:,i,k)  
                    ! print *, 'pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_pressure(:,i,k)', pod_coef_all(s,i+(dimen+1)*nsvd) !,leftsvd_pressure(:,i,k)
                  !  print *, 'nodes_mesh_subdomain(:,s)%pressure', nodes_mesh_subdomain(1,1)%pressure, nodes_mesh_subdomain(1,1)%id
                      nodes_mesh_subdomain(:,s)%pressure=nodes_mesh_subdomain(:,s)%pressure+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_pressure(:,i,k)
                      nodes_mesh_subdomain(:,s)%volumefraction=nodes_mesh_subdomain(:,s)%volumefraction+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_volumefraction(:,i,k) 
                     !pressure_subdomain(:)= interpolate_pressure(:,s,k)+pod_coef_all(s,i+dimen*nsvd)*leftsvd_pressure(:,i,k)
                     !volumefraction_subdomain(:)=interpolate_volumefraction(:,s,k)+pod_coef_all(s,i+(dimen+1)*nsvd)*leftsvd_volumefraction(:,i,k)
                    ! nodes_mesh(:,s)%pressure=interpolate_pressure(:,s,k)+pod_coef_all(s,i+dimen*nsvd)*leftsvd_pressure(:,i,k)
                    ! nodes_mesh(:,s)%volumefraction=interpolate_volumefraction(:,s,k)+pod_coef_all_obv(s,i+(dimen+1)*nsvd,k)*leftsvd_volumefraction(:,i,k)
                  else
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     ! print *, 'nodes_mesh_subdomain(:,s)%velocity(d)', nodes_mesh_subdomain(:,s)%velocity(1)
   

     do i=1, u_nodes
        do d=1,dimen
        velocity_field%val(d,i)= velocity_field%val(d,i)+nodes_mesh_subdomain(i,s)%velocity(d) ! allocate(snapmean_velocity(u_nodes,dimen,numphase))
        enddo
     enddo

     do i=1, p_nodes !nodes_mesh(i,s)%sequence
       pressure_field%val(i)=pressure_field%val(i)+ nodes_mesh_subdomain(i,s)%pressure
       volumefraction_field%val(i)=  volumefraction_field%val(i)+ nodes_mesh_subdomain(i,s)%volumefraction
     enddo
   
   !   do i=1, unodes_sub
    !    do d=1,dimen
     !   velocity_field%val(d,nodes_mesh(i,s)%id)=nodes_mesh(i,s)%velocity(d)  ! allocate(snapmean_velocity(u_nodes,dimen,numphase))
    !  enddo
   !  enddo
  ! print *, 'pressure_field%val(nodes_mesh(i,s)%id)', nodes_mesh(i,s)%id, pressure_field%val(:)
      if (ii .eq. total_subdomain) then
       call insert(state_show(k), velocity_field, "Velocity")
     call insert(state_show(k), pressure_field, "Pressure") 
     call insert(state_show(k), volumefraction_field, "Volumefraction")!c
    endif
  !  endif !if (ii .eq. number_subdomainx) then
      enddo !do k=1, numphase
   ! if (ii .eq. number_subdomainx) then !    if (ii .eq. 1) then 
     dump_no=s  
   ...
   ! enddo !Do iiteration 
      ! call write_state(dump_no, state(1,:,1))
     ...
     deallocate(xd)
     deallocate(w)
     deallocate(velocity_subdomain)
     deallocate(pressure_subdomain)
     deallocate(volumefraction_subdomain) 
    end subroutine non_intrusive_subdomains
 
   subroutine non_intrusive_subdomain(ii, unodes_sub, nodes_mesh_subdomain)
    type(vector_field), pointer :: velocity_field, velocity_initial
    type(scalar_field), pointer :: pressure_field, volumefraction_field, pressure_initial, volumefraction_initial
    type(state_type):: state_initial
    integer, intent(in) :: unodes_sub
   ...
   allocate (xd(podnum,nd))
     ...
     if(.false.) then
     ! setting the initial begin
       write(filename, '(a, a)') trim(simulation_name)//'_1', ".vtu"
       call vtk_read_state(filename, state_initial, quadrature_degree)
       velocity_initial => extract_vector_field(state_initial, "Velocity") !velocity%val(d,:)
       pressure_initial => extract_scalar_field(state_initial, "Pressure")
       volumefraction_initial=> extract_scalar_field(state_initial, "Pressure")!c
       do i=1,size(POD_state,1)
         do j=1,dimen !velocity%dim
             pod_coef_old(i+nsvd*(j-1))=dot_product(leftsvd_velocity(:,i,j,1),(velocity_initial%val(j,:)-snapmean_velocity(:,j,1)))
         enddo
             pod_coef_old(i+nsvd*dimen)=dot_product(leftsvd_pressure(:,i,1),(pressure_initial%val(:)-snapmean_pressure(:,1)))
             pod_coef_old(i+nsvd*(dimen+1))=dot_product(leftsvd_volumefraction(:,i,1),(volumefraction_initial%val(:)-snapmean_volumefraction(:,1)))
       enddo
     ! setting the initial over
     endif
          ...
          ! print *, 'size(velocity_field%val,2)',  size(velocity_field%val,2)
           ...
           do i=1,nsvd
                ...
                !interpolate_velocity(nodes_mesh(j,s)%sequence,s,k,1)=nodes_mesh(j,s)%velocity(k)
                       if( s <= total_dumps) then
                         ! velocity_subdomain(d,:)=interpolate_velocity(:,s,d,k)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                        ! velocity_subdomain(d,:)=nodes_mesh_subdomain(:,s)%velocity(d)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                        nodes_mesh_subdomain(:,s)%velocity(d)=nodes_mesh_subdomain(:,s)%velocity(d)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                ! print *, 'leftsvd_velocity(:,i,d,k)',  size(leftsvd_velocity,1), leftsvd_velocity(:,i,d,k) 
                         ! print *, 'nodes_mesh', size(nodes_mesh,1)
                         ! nodes_mesh(:,s)%velocity(d)=interpolate_velocity(:,s,d,k)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     ! print *, 'nodes_mesh_subdomain(:,s)%velocity(d)', nodes_mesh_subdomain(:,s)%velocity(1)
     do i=1, unodes_sub  
      temp_id=nodes_mesh_subdomain(i,s)%id 
      nodes_mesh_subdomain(i,s)%domain_id=ii
      nodes_mesh(temp_id,s)%pressure=nodes_mesh_subdomain(i,s)%pressure
      nodes_mesh(temp_id,s)%volumefraction=nodes_mesh_subdomain(i,s)%volumefraction
      do d=1, dimen
       nodes_mesh(temp_id,s)%velocity(d) = nodes_mesh_subdomain(i,s)%velocity(d)
      enddo
     enddo 
     ...
     do i=1, pnodes_sub !nodes_mesh(i,s)%sequence
       pressure_field%val(nodes_mesh(i,s)%id)=  nodes_mesh(i,s)%pressure
       volumefraction_field%val(nodes_mesh(i,s)%id)=  nodes_mesh(i,s)%pressure
     enddo
  !   print *, 'pressure_field%val(nodes_mesh(i,s)%id)', nodes_mesh(i,s)%id, pressure_field%val(:)
   ! if (ii .eq. number_subdomainx) then
   !  do i=1, pnodes_sub !nodes_mesh(i,s)%sequence
   !    pressure_field%val(nodes_mesh(i,s)%id)=  nodes_mesh(i,s)%pressure
    !   volumefraction_field%val(nodes_mesh(i,s)%id)=  nodes_mesh(i,s)%pressure
    ! enddo
     call insert(state_show(k), pressure_field, "Pressure") 
     call insert(state_show(k), volumefraction_field, "Pressure")!c
   !   do i=1, unodes_sub
    !    do d=1,dimen
     !   velocity_field%val(d,nodes_mesh(i,s)%id)=nodes_mesh(i,s)%velocity(d)  ! allocate(snapmean_velocity(u_nodes,dimen,numphase))
    !  enddo
  ...
  !  endif !if (ii .eq. number_subdomainx) then
     ...
     if (ii .eq. number_subdomainx) then
     dump_no=s
     ! call print_state(state_show(1))!state(:,s,1)
     ...
     ! call write_state(dump_no, state_show)
      ...
      !call vtk_write_state(filename=trim(simulation_name)//"VelocityMesh", index=dump_no, model="VelocityMesh", state=state_show(:))
      !call vtk_write_state(filename=trim(simulation_name)//"PressureMesh", index=dump_no, model="Mesh", state=state(:,s,1))
      !print *, 'sss', s
      if (s .eq.1) then
        call set_option('/simulation_name', trim(simulation_name)//'_POD')
      endif
    ...
    ENDDO ! Do s=1, total_timestep
     ...
     deallocate(pressure_subdomain)
     deallocate(volumefraction_subdomain) 
    end subroutine non_intrusive_subdomain

 subroutine non_intrusive_subdomainss(ii, nodes_mesh_subdomain)
    ! other neighbouring subdomains' solutions are known, therefore, put next timelevel's solutions of neighbouring subdomains and current into pod_coef_all, when training rbf, inputs including nei
    type(vector_field), pointer :: velocity_field, velocity_initial
    type(scalar_field), pointer :: pressure_field, volumefraction_field, pressure_initial, volumefraction_initial
    type(state_type):: state_initial
    !integer, intent(in) :: unodes_sub
   ...
   ! endif
    ...
    podnum =m
     ...
     open(unit=61,file="coef_pod_all_obv"//"_"//trim(simulation_name)//"subdomain"//int2str(ii)) ! coef_pod_all_obv includes coef_pod from other neighbouring subdomains
     read(61,*)((pod_coef_all(j,k),k=1,podnum),j=1,total_dumps)
     close(61)
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !  endif !if (ii .eq. number_subdomainx) then
   ...
   ! enddo !Do iiteration 
     ...
     deallocate(pressure_subdomain)
     deallocate(volumefraction_subdomain) 
    end subroutine non_intrusive_subdomainss
 ...
 subroutine nonintrusive_snapshots_main()
    ...
    allocate(interpolate_volumefraction(p_nodes,snapshots,numphase))
     ...
     interpolate_pressure=snapmatrix_pressure(1,:,:,:)
     interpolate_volumefraction=snapmatrix_volumefraction(1,:,:,:)
     do p = 1, numphase
     do i=1, snapshots
       do d=1, dimen
  ...
  subroutine read_subdomain_id(state_id)

    type(state_type), intent(inout), dimension(:), allocatable :: state_id
    character(len=1024) :: filename
    ...
    allocate(state_id(total_subdomain ))
    call get_option('/geometry/quadrature/degree', quadrature_degree) 

     do i=1, total_subdomain
       ! write(filename, '(a, i0, a)') 'test_flredecomp_VelocityMesh/test_flredecomp_VelocityMesh_', i-1 ,".vtu"
          write(filename, '(a, i0, a)') 'testwgt1_flredecomp_VelocityMesh/testwgt1_flredecomp_VelocityMesh_', i-1 ,".vtu"
       call vtk_read_state(filename, state_id(i), quadrature_degree)
       print *, 'filename', filename
     end do 
     
  end subroutine read_subdomain_id
  ...
  subroutine read_input_states(state)
    ...
    allocate(state(no_smolyak_nodes,total_timestep,numphase))
    ...
    do i=1, total_dumps
     ...
     !initialise the state after total_dumps
     !  do i= total_dumps, total_timestep
     !  write(filename, '(a, i0, a)') trim(simulation_name)//'_', (total_dumps-total_timestep+i)*dump_sampling_period,".vtu"
     !   call vtk_read_state(filename, state(k,i,j), quadrature_degree)
     ! end do
    enddo
   enddo
  ...
  subroutine read_input_states_lsbu(state)
    ...
    enddo
   enddo
  end subroutine read_input_states_lsbu
  ...
  !  total_timestep=total_dumps
   ...
   allocate (xd(podnum,nd))
     ...
     !  print *,'differnce',  pod_coef_old-pod_coef_all(1,:)
       pod_coef_old=pod_coef_all(1,:)
    call set_option('/simulation_name', trim(simulation_name)//'_PODfinal')
      call get_option('/simulation_name',simulation_name)
      ...
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
       ...
       !nomean
         ...
         snapmean_volumefraction=0
           ...
           do i=1,nsvd
                do d=1,velocity_field%dim
                   !  print *, 'difference', pod_coef_new(i+(d-1)*nsvd)-pod_coef_all_obv(s,i+(d-1)*nsvd,k)
                       if( s <= total_dumps) then
                       !  velocity_field%val(d,:)=velocity_field%val(d,:)+pod_coef_all_obv(s,i+(d-1)*nsvd,k)*leftsvd_velocity(:,i,d,k)
                          velocity_field%val(d,:)=velocity_field%val(d,:)+pod_coef_all(s,i+(d-1)*nsvd)*leftsvd_velocity(:,i,d,k)
                       else
                ...
                enddo
                     if( s <= total_dumps) then
                        ...
                        pressure_field%val(:)=pressure_field%val(:)+pod_coef_all(s,i+dimen*nsvd)*leftsvd_pressure(:,i,k)
           ...
           enddo
       ! print *,'beforepressure', pressure_field%val(2), snapmean_pressure(2,k),  leftsvd_pressure(2,2,k)
       pressure_field%val=pressure_field%val(:)+ snapmean_pressure(:,k)
       ! print *,'afterpressure', pressure_field%val(2), snapmean_pressure(2,k),  leftsvd_pressure(2,2,k)
     call insert(state_show(k), pressure_field, "Pressure") 
    ...
    ! call write_state(dump_no, state_show)
      ...
      !  call calculate_diagnostic_variables_new( state_show, exclude_nonrecalculated = .true. )
      pod_coef_old=pod_coef_new ! iteration
    ENDDO ! Do s=1, total_timestep
      ! call write_state(dump_no, state(1,:,1))
  ...
  subroutine put_vtu_to_states(state,state_p)
    ...
    numphase=option_count("/material_phase")
    allocate(state(no_smolyak_nodes,total_timestep,numphase))
    allocate(state_p(no_smolyak_nodes,total_timestep,numphase))
    ...
    enddo
     ...
     ! initialise the state using other vtu files
    do i= total_dumps, total_timestep

             write(filename, '(a, i0, a,a)')  trim(int2str(k))//'_'//trim(phase_name)//'_VelocityMesh_', (total_timestep-total_dumps+i)*dump_sampling_period,'_checkpoint',".vtu"
             call vtk_read_state(filename, state(k,i,j), quadrature_degree)
             print *,'filename=', filename
             write(filename, '(a, i0, a,a)')  trim(int2str(k))//'_'//trim(phase_name)//'_PressureMesh_', (total_timestep-total_dumps+i)*dump_sampling_period,'_checkpoint',".vtu"
             call vtk_read_state(filename, state_p(k,i,j), quadrature_degree)
              print *,'filename=', filename
   ...
   subroutine retrieve_subdomain_id_from_state_id(state_id, ii, snapmatrix_id)
     type(state_type), intent(in) :: state_id
     type(scalar_field), pointer :: ids
     integer, dimension(:), intent(out), allocatable :: snapmatrix_id
     integer :: u_nodes_subdomain, p_nodes_subdomain, ii,j
     type(vector_field), pointer :: velocity_subdomain 
     ids => extract_scalar_field(state_id, "id")
     
     p_nodes_subdomain = node_count(ids)
     !print *, "p_nodes_subdomain", p_nodes_subdomain
     allocate(snapmatrix_id(p_nodes_subdomain))     
     snapmatrix_id(:)=ids%val(:)
     ! open(unit=61,file=int2str(ii))
     ! read(61,*)(snapmatrix_id(j),j=1,p_nodes_subdomain)
     ! close(61)
     !print *,  'snapmatrix_id(:)', ii, snapmatrix_id(1), snapmatrix_id(2)
   end subroutine retrieve_subdomain_id_from_state_id
   ...
   subroutine retrieve_nodesvalue_from_states(state, snapmatrix_velocity, snapmatrix_pressure, snapmatrix_volumefraction)
    ...
    volumefraction=> extract_scalar_field(state(1,1,1), "Pressure")! (state_p(1,1,1), "PhaseVolumeFraction")!c
    dimen=velocity%dim
    p_nodes=node_count(pressure)
    u_nodes=node_count(velocity)
   ...
   do p = 1, numphase
    do i = 1, snapshots
      ! print *, 'iiiii', i
       ...
       pressure => extract_scalar_field(state(k,i,p), "Pressure")
       volumefraction=> extract_scalar_field(state(k,i,p), "Pressure")!c
       do d = 1, dimen
...
end program subdomain_non_intrusive_vtu_lsbu
