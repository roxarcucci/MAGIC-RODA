...
contains 
  ...
  !  interpolate_volumefraction(:,snapshots,:)=snapmean_volumefraction(:,:)!jcp-revi
    ...
    do i=1,nsvd 
      ...
      !call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisPressure", index=dump_no, model="VelocityMesh", state=pod_state_p(i,:,ph))
      !call vtk_write_state(filename=trim(simulation_name)//"_"//trim(phase_name)//"_PODBasisVolumefraction", index=dump_no, model="VelocityMesh", state=pod_state_v(i,:,ph))
    ...
    ! visulisation
     ...
     do k=1, numphase 
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
        velocity_field => extract_vector_field(state(1,s,1), "phase1::Velocity" )
        pressure_field => extract_scalar_field(state(1,s,1), "Component1::ComponentMassFractionPhase1" )
        volumefraction_field=> extract_scalar_field(state(1,s,1), "Component1::ComponentMassFractionPhase1")!SolidConcentration" )  ! change
 ...
 !!!  for case has the same mesh like P1_P1
  ...
  subroutine put_pvtu_to_states(state,state_p)
   ...
   ! integer :: numphase
    ...
    !substract gyre_0.vtu
    total_dumps=30!count_dumps_pvtu(dump_sampling_period)
   ! numphase=option_count("/material_phase")
    allocate(state(no_smolyak_nodes,total_dumps,numphase))
    allocate(state_p(no_smolyak_nodes,total_dumps,numphase))
    ...
    if (.false.) then
     do i=total_dumps+1, total_dumps+30
  ...
  !  numphase=option_count("/material_phase")
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       pressure => extract_scalar_field(state(k,i,1), "Component1::ComponentMassFractionPhase1")
       print *, 'p_nodes=node_count(pressure)', p_nodes
       volumefraction=> extract_scalar_field(state(k,i,p), "Component1::ComponentMassFractionPhase1")! change
       do d = 1, dimen 
 ...
 subroutine form_podstate_velocity(state, pod_state, leftsvd_u, leftsvd_p, snapmean_u, snapmean_p)
    ...
    do i = 1,POD_num
      ...
      ! pod_mesh => extract_mesh(state(1), "Mesh")
       ...
       velocity => extract_vector_field(state(1,p), "phase1::Velocity")
