...
program non_intrusive_vtu_predict
  use spud
  use fields
  use state_module
...
!#include "finclude/petsc.h"
  ...
  real, dimension(:), allocatable :: snapmatrix_time
  integer :: nsvd
...
!  call adaptive_timestep_interpolant_main()
...
contains 
  ...
  subroutine adaptive_timestep_interpolant_main()
    ...
    ! Read state from .flml file,  (snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase))
    ...
    call retrieve_time_from_states(state, snapmatrix_time)  
    call adaptive_timestep_interpolant()
    deallocate(snapmatrix_velocity)
    deallocate(snapmatrix_pressure) 
    deallocate(snapmatrix_volumefraction)  
    deallocate(snapmatrix_time)
    
   end subroutine adaptive_timestep_interpolant_main 
 
   subroutine adaptive_timestep_interpolant()
    type(vector_field), pointer :: velocity_field
    ...
    real, allocatable :: wm(:,:)
    real :: r0
    real , allocatable :: xd(:,:), xd_lq(:,:), smoylak_all(:,:) 
    real , allocatable :: xi(:,:)
    ...
    real :: maxall,minall
    integer :: m,ll,kk,u_nonods,l,nd, podnum,ni,p_nonods 
    total_timestep=total_dumps 
    m = 1!(dimen+2)*nsvd*numphase
    allocate(w(m)) 
   ! (snapmatrix_velocity(no_smolyak_nodes,u_nodes,snapshots,dimen,numphase)) snapmatrix_velocity(k,:,i,d,p)=velocity%val(d,:)
   ...
   allocate (xd(m,nd))  
     do j=1,m
         do k=1,nd
           xd(j,k)= snapmatrix_time(k)
         enddo
       enddo 
      maxall=snapmatrix_time(nd)
      minall=snapmatrix_time(1)
     r0 = ( maxall - minall ) / real ( nd, kind = 8 )

!!!!  no smolyak and numphase
 DO s=1, total_timestep 
       xi(:,1)= s*snapmatrix_time(nd)/300  !300 is the new number of vtu (will be generated)  
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  velocity
 do d =1, dimen
   do u_nonods=1, u_nodes  
     do l=1, m
        do k=1,nd
          fd(k)=snapmatrix_velocity(1,u_nonods,k,d,1)  ! target function value   
        enddo 
     ...
     do l=1, m   
      ...
      ! PHI1 evaluates the multiquadric radial basis function. terrible.! 
       velocity_field%val(d,u_nonods)=fi(1)  
     enddo  !do l=1, m  
    enddo
   enddo 
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! velocity            
    
     do p_nonods=1, p_nodes  
     do l=1, m
        do k=1,nd
          fd(k)=snapmatrix_pressure(1,p_nonods,k,1)  ! target function value   
        enddo 
      call rbf_weight ( m, nd, xd, r0, phi4, fd, wm(:,l) )
     enddo 
     ...
     do l=1, m   
      ...
      ! PHI2 evaluates the inverse multiquadric radial basis function.,! 
      ! PHI1 evaluates the multiquadric radial basis function. terrible.! 
       pressure_field%val(p_nonods)=fi(1)  
     enddo  !do l=1, m  
    enddo
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! pressure
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! volumefraction
     ...
     do l=1, m
        do k=1,nd
          fd(k)=snapmatrix_volumefraction(1,p_nonods,k,1)  ! target function value   
        enddo 
      call rbf_weight ( m, nd, xd, r0, phi4, fd, wm(:,l) )
     enddo 
     ...
     do l=1, m   
      ...
      ! PHI2 evaluates the inverse multiquadric radial basis function.,! 
      ! PHI1 evaluates the multiquadric radial basis function. terrible.! 
        volumefraction_field%val(p_nonods)=fi(1)  
     enddo  !do l=1, m  
    enddo
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! volumefraction
      call print_state(state(1,1,1)) 
     
       call set_option('/simulation_name', trim(simulation_name)//'_interpolate_POD') 
       call vtk_write_state(filename=trim(simulation_name), index=s, model="Mesh", state=state(:,s,1))
      ! call vtk_write_state(filename=trim(simulation_name), index=s, model="PressureMesh", state=state(:,s,1)) 
     ...
     deallocate(xd)
     deallocate(w)
    end subroutine adaptive_timestep_interpolant
 ...
 !!!  for case has the same mesh like P1_P1
    ...
    total_dumps=380!count_dumps(dump_sampling_period)-1
    allocate(state(no_smolyak_nodes,total_dumps+10,numphase)) 
  ...
  subroutine put_pvtu_to_states(state,state_p)
    ...
    character(len=1024) :: filename
     type(scalar_field) ::pressure,  volumefraction, time_level
    integer :: dump_sampling_period, quadrature_degree
    integer :: i,j,k,stable_dumps
   ...
   ! integer :: numphase
    ...
    total_dumps=200!count_dumps_pvtu(dump_sampling_period)
   ! numphase=option_count("/material_phase")
    ...
    do k=1, no_smolyak_nodes
     ...
     do i=1, total_dumps
          ...
          !   print *,'filename=', filename
             ...
             print *,'filename=', filename  
               call print_state(state(1,1,1))
              !time_level=extract_scalar_field(state(k,i,j),"Time")
            !  print *, 'timeeeeeeeeeeeeeeeeeee', time_level%val(1)
    ...
    do k=1, no_smolyak_nodes
     ...
     do i=1, total_dumps
         ! if(numphase .eq. 1) then
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_phase1_'//'VelocityMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state(k,i,j), quadrature_degree)
             print *,'filename=', filename  
             write(filename, '(a, i0, a,a)')  trim(simulation_name)//'_Component1_'//'PressureMesh_', (i)*dump_sampling_period,'_checkpoint',".vtu" 
             call vtk_read_state(filename, state_p(k,i,j), quadrature_degree) 
              print *,'filename=', filename
        !  else
        ...
        pressure=extract_scalar_field(state_p(k,i,j),"Component1::ComponentMassFractionPhase1")
       call insert(state(k,i,j),  pressure, name="Component1::ComponentMassFractionPhase1")
    
       volumefraction=extract_scalar_field(state_p(k,i,j),"Component1::ComponentMassFractionPhase1")
        call insert(state(k,i,j),  volumefraction, name="Component1::ComponentMassFractionPhase1")
       ! call insert(state(k,i,j), pressure_field, "Pressure")   change 5 places if changing one field
    enddo
    enddo  
    enddo
  ...
  !  numphase=option_count("/material_phase")
   ...
   !  deallocate(pod_state)
    ...
    subroutine retrieve_time_from_states(state, snapmatrix_time)
   ...
   !!< 
    ...
    !time => extract_scalar_field(state(1,1,1), "Time")  
    snapshots=size(state(1,:,1)) 
    allocate(snapmatrix_time(snapshots))  
    snapmatrix_time=0.0  
    do i = 1, snapshots 
       time=> extract_scalar_field(state(1,i,1), "Time")! change 
       snapmatrix_time(i)=time%val(1) 
    end do
 
   end subroutine retrieve_time_from_states
   ...
   ! velocity => extract_vector_field(state(1,1,1), "phase1::Velocity")
    !pressure => extract_scalar_field(state(1,1,1), "Component1::ComponentMassFractionPhase1") !(state_p(1,1,1), "Pressure") 
   ! volumefraction=> extract_scalar_field(state(1,1,1), "Component1::ComponentMassFractionPhase1")!(state_p(1,1,1), "SolidConcentration")! change
    ...
    do i = 1,POD_num
     ...
     print *, 'form_podstate_volumefraction', i
      ...
      ! pod_pmesh =>extract_mesh(state_p(1,p), "PressureMesh")  !change to pressuremesh from mesh
        pod_pmesh =>extract_mesh(state_p(1,p), "Mesh")  !change to pressuremesh from mesh
       pod_positions => extract_vector_field(state_p(1,p), "Coordinate") 
...
end program non_intrusive_vtu_predict
