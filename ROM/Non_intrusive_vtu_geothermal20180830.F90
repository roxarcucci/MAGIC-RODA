    ...
    ! visulisation
     ...
     do k=1, numphase 
      ! velocity_field => extract_vector_field(state_show(1), "Velocity" )
        ...
        pressure_field => extract_scalar_field(state(1,s,1), "phase1::Pressure" )
        volumefraction_field=> extract_scalar_field(state(1,s,1), "phase1::Temperature")!SolidConcentration" )  ! change
 ...
 !!!  for case has the same mesh like P1_P1
  ...
  subroutine put_vtu_to_states(state,state_p)
   ...
   ! integer :: numphase
    ...
    total_dumps=230!count_dumps(dump_sampling_period)
   ! numphase=option_count("/material_phase")
    allocate(state(no_smolyak_nodes,total_dumps,numphase))
    ...
    do k=1, no_smolyak_nodes
     ...
     do i=1, total_dumps
      ...
      !  call insert(state(k,i,j), pressuremesh, name="PressureMesh") 
        ...
        pressure=extract_scalar_field(state_p(k,i,j),"phase1::Pressure")
       call insert(state(k,i,j),  pressure, name="phase1::Pressure")
    
       volumefraction=extract_scalar_field(state_p(k,i,j),"phase1::Temperature")
        call insert(state(k,i,j),  volumefraction, name="phase1::Temperature")
  ...
  !  numphase=option_count("/material_phase")
   ...
   ! print *, 'sizeofstate', size(state,1), size(state,2), size(state,3)
    ...
    pressure => extract_scalar_field(state(1,1,1), "phase1::Pressure") !(state_p(1,1,1), "Pressure") 
    volumefraction=> extract_scalar_field(state(1,1,1), "phase1::Temperature")!(state_p(1,1,1), "SolidConcentration")! change
    dimen=velocity%dim
   ...
   do p = 1, numphase
    do i = 1, snapshots
       ...
       pressure => extract_scalar_field(state(k,i,1), "phase1::Pressure")
       print *, 'p_nodes=node_count(pressure)', p_nodes
       volumefraction=> extract_scalar_field(state(k,i,p), "phase1::Temperature")! change
       do d = 1, dimen 
